...
from .context_awareness import get_context_engine, SmartRecommendation, RecommendationLevel
...
class InteractiveMenu:
    ...
    def __init__(self, bot: commands.Bot, menu_type: MenuType, style: MenuStyle = MenuStyle.DETAILED):
        self.bot = bot
        self.menu_type = menu_type
        self.style = style
        self.timeout = 300  # 5分鐘超時
        
    async def create_embed(self, title: str, description: str = "", color: int = 0x3498db) -> discord.Embed:
        """創建標準化 Embed"""
        embed = discord.Embed(
            title=title,
        ...
        embed.set_footer(text="Potato Bot • Phase 7 GUI System", icon_url=self.bot.user.avatar.url if self.bot.user else None)
        return embed
...
class MainMenuView(discord.ui.View):
    ...
    async def ai_assistant(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 只有選單發起者可以使用此功能", ephemeral=True)
            return
        ...
        try:
            ...
            try:
                ...
                for guild in self.bot.guilds:
                    tickets = await ticket_dao.get_guild_tickets(guild.id, status=['open', 'in_progress'])
            ...
            try:
                ...
                embed.add_field(name="💾 記憶體", value=f"{memory_used}MB / {memory_total}MB", inline=True)
            except ImportError:
...
class TicketMenuView(discord.ui.View):
    ...
    async def create_main_embed(self) -> discord.Embed:
        ...
        try:
            ...
            # 獲取系統設定
            settings = await ticket_dao.get_settings(self.bot.guilds[0].id if self.bot.guilds else 0)
    ...
    async def create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # 獲取票券設定
            ticket_dao = TicketDAO()
            settings = await ticket_dao.get_settings(interaction.guild.id) if interaction.guild else {}
...
class SmartChatModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("❌ 抱歉，AI 助手暫時無法回應。請稍後再試。", ephemeral=True)
...
class CreateTicketModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="📝 描述",
            value=self.description_input.value[:200] + ("..." if len(self.description_input.value) > 200 else ""),
            inline=False
        )
        ...
        try:
            ...
            if active_votes:
                ...
                for vote in recent_votes:
                    ...
                    vote_list.append(f"• **{vote['title'][:30]}...**\n  {status} | {vote['total_votes']} 票")
    ...
    async def preview_welcome(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if cog and hasattr(cog, 'test_welcome_message'):
            ...
            try:
                ...
                result = await welcome_manager.test_welcome_message(interaction.guild, interaction.user)
                
                embed = discord.Embed(
                    title="🎆 歡迎訊息預覽",
                    description=result.get('message', f"歡迎 {interaction.user.mention} 來到 {interaction.guild.name}！"),
                    color=0x2ecc71
                )
                embed.add_field(
                    name="📍 頻道",
...
class SettingsMenuView(discord.ui.View):
    ...
    async def permission_settings(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("❌ 需要管理員權限才能查看此設定", ephemeral=True)
            return
...
class EntertainmentMenuView(discord.ui.View):
    ...
    async def entertainment_center(self, interaction: discord.Interaction, button: discord.ui.Button):
        """開啟娛樂中心"""
        # 調用娛樂模組的主功能
...
class MenuSystemManager:
    ...
    async def show_main_menu(self, interaction: discord.Interaction) -> None:
        ...
        # 獲取情境化選單選項
        contextual_options = await context_engine.get_contextual_menu_options(str(user_id), guild_id)
