...
class GitHubTokenDebug(commands.Cog):
    ...
    async def debug_github_token(self, ctx):
        ...
        if not github_token:
            embed.add_field(name="❌ 環境變數", value="GITHUB_TOKEN 未設置", inline=False)
        else:
            # 驗證 token 格式
        ...
        if github_token:
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(repo_url, headers=headers) as response:
                        if response.status == 200:
                            ...
                            embed.add_field(
                                name="❌ 倉庫存取", value=f"狀態碼: {response.status}", inline=True
                            )
                ...
                embed.add_field(name="❌ API 測試失敗", value=f"錯誤: {str(e)[:100]}", inline=False)
