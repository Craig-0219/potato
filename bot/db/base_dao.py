...
class BaseDAO(ABC):
    ...
    async def execute_query(self, query: str, params: Tuple = None, 
                           ...
                           dictionary: bool = False) -> Any:
        """執行 SQL 查詢的通用方法"""
        await self._ensure_initialized()
        
        try:
            async with self.db.connection() as conn:
                cursor_class = conn.cursor if not dictionary else lambda: conn.cursor(aiomysql.DictCursor)
    ...
    async def get_all(self, where_clause: str = None, params: Tuple = None,
    ...
    async def paginate(self, page: int = 1, page_size: int = 10,
                      where_clause: str = None, params: Tuple = None,
                      order_by: str = "id DESC") -> Tuple[List[Dict[str, Any]], int]:
        """分頁查詢"""
        if not self.table_name:
            raise NotImplementedError("table_name 必須被設定")
    ...
    def _get_from_cache(self, key: str) -> Any:
        ...
        # 檢查是否過期
        if (datetime.now(timezone.utc) - cached_item['timestamp']).total_seconds() > self._cache_timeout:
            del self._cache[key]
            return None
    ...
    async def bulk_insert(self, data_list: List[Dict[str, Any]], 
    ...
    async def bulk_update(self, updates: List[Tuple[Union[int, str], Dict[str, Any]]], 
    ...
    def validate_data(self, data: Dict[str, Any], required_fields: List[str] = None) -> Tuple[bool, str]:
        """驗證資料完整性"""
        if required_fields:
