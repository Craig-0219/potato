...
class SecurityDAO(BaseDAO):
    ...
    async def get_active_alerts(self, guild_id: int = None, severity: str = None) -> List[Dict[str, Any]]:
        """獲取活躍的安全警報"""
        try:
    ...
    async def update_session_activity(self, session_id: str, activity_count: int = 1, risk_score: float = 0.0):
        """更新會話活動"""
        try:
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
    ...
    async def get_compliance_reports(self, guild_id: int, standard: str = None) -> List[Dict[str, Any]]:
        """獲取合規報告列表"""
        try:
