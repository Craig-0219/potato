...
class WebhookDAO(BaseDAO):
    ...
    async def get_webhooks(self, guild_id: Optional[int] = None, status: Optional[str] = None) -> List[Dict[str, Any]]:
        """獲取Webhook列表"""
        try:
            conditions = []
            params = []
    ...
    async def get_webhook_logs(self, webhook_id: Optional[str] = None, days: int = 7, 
    ...
    async def update_webhook_statistics(self, webhook_id: str, execution_time: float, success: bool):
        """更新Webhook統計"""
        try:
