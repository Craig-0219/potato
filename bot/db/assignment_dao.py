...
class AssignmentDAO:
    ...
    async def update_staff_workload(self, guild_id: int, staff_id: int, 
                                  current_tickets: int = None,
                                  increment_assigned: bool = False,
                                  increment_completed: bool = False,
                                  completion_time_minutes: int = None) -> bool:
        """更新客服人員工作量"""
        await self._ensure_initialized()
        try:
            # 確保工作量記錄存在
    ...
    async def get_available_staff(self, guild_id: int, max_concurrent: int = 5) -> List[Dict[str, Any]]:
        """取得可用的客服人員（根據工作量排序）"""
        await self._ensure_initialized()
        try:
            async with self.db.connection() as conn:
    ...
    async def record_assignment(self, ticket_id: int, assigned_from: Optional[int], 
                              assigned_to: int, assigned_by: int,
                              reason: str = "manual", method: str = "manual") -> bool:
        """記錄指派歷史"""
        await self._ensure_initialized()
        try:
            async with self.db.connection() as conn:
    ...
    async def add_staff_specialty(self, guild_id: int, staff_id: int, 
                                specialty_type: str, skill_level: str = "intermediate") -> bool:
        """添加客服專精"""
        await self._ensure_initialized()
        try:
            async with self.db.connection() as conn:
    ...
    async def find_specialty_staff(self, guild_id: int, specialty_type: str, 
                                 min_skill_level: str = "intermediate") -> List[Dict[str, Any]]:
        """根據專精查找客服人員"""
        await self._ensure_initialized()
        try:
    ...
    async def get_assignment_rule(self, guild_id: int, ticket_type: Optional[str] = None, 
                                 priority_level: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """取得適用的指派規則"""
        await self._ensure_initialized()
        try:
            async with self.db.connection() as conn:
    ...
    async def create_assignment_rule(self, guild_id: int, rule_name: str,
                                   ticket_type: Optional[str] = None,
                                   priority_level: Optional[str] = None,
                                   assignment_method: str = "auto_least_workload",
                                   max_concurrent_tickets: int = 5) -> bool:
        """建立指派規則"""
        await self._ensure_initialized()
        try:
            async with self.db.connection() as conn:
