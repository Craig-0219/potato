...
class WelcomeDAO(BaseDAO):
    ...
    async def upsert_welcome_settings(self, guild_id: int, settings: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            auto_roles_json = json.dumps(settings.get('auto_roles', [])) if settings.get('auto_roles') else None
    ...
    async def log_welcome_event(self, guild_id: int, user_id: int, username: str,
                               action_type: str, welcome_sent: bool = False,
                               roles_assigned: List[int] = None, dm_sent: bool = False,
        ...
        try:
            ...
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (
                        guild_id, user_id, username, action_type,
                        welcome_sent, roles_json, dm_sent, error_message
                    ))
    ...
    async def get_welcome_logs(self, guild_id: int, limit: int = 50, 
    ...
    async def update_system_settings(self, guild_id: int, settings_type: str, 
                                   settings: Dict[str, Any]) -> bool:
        """更新系統設定"""
        try:
            valid_types = ['general_settings', 'channel_settings', 'role_settings', 
                          'notification_settings', 'feature_toggles', 'custom_settings']
