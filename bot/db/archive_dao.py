...
class ArchiveDAO(BaseDAO):
    ...
    async def archive_old_tickets(self, guild_id: int, days_old: int = 90, batch_size: int = 1000) -> Dict[str, int]:
        """歸檔舊票券"""
        try:
            async with self.db.connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    for ticket in tickets_to_archive:
                        ...
                        await cursor.execute(archive_query, (
                            ticket['id'],
                            guild_id,
    ...
    async def archive_old_votes(self, guild_id: int, days_old: int = 90, batch_size: int = 1000) -> Dict[str, int]:
        """歸檔舊投票"""
        try:
            async with self.db.connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    for vote in votes_to_archive:
                        ...
                        options_data = json.loads(vote['options_data']) if vote['options_data'] else []
                        responses_data = [
                            {
                        ...
                        results_data = {
                            ...
                            'participation_rate': len(responses) / max(vote['total_voters'], 1) * 100
                        }
                ...
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    for activity in user_activities:
                        activity_data = {
                            ...
                            'total_interactions': activity['tickets_count'] + activity['votes_count'],
    ...
    async def cleanup_old_data(self, guild_id: int, data_type: str, days_old: int = 90, delete_after_archive: bool = True) -> Dict[str, int]:
        """清理舊資料"""
        try:
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if data_type == 'tickets':
                        ...
                        # 清理舊日誌
                        tables_to_clean = [
                            'ticket_logs', 'vote_logs', 'cleanup_logs', 
                            'webhook_logs', 'export_logs'
                        ]
                    ...
                    VALUES (%s, %s, %s, %s)
                    """
                    await cursor.execute(log_query, (guild_id, data_type, deleted_count, cutoff_date))
                    await conn.commit()
