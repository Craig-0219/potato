...
class TicketListener(commands.Cog):
            ...
            if keyword in content_lower:
                ...
                if ticket_info.get("priority", "medium") != "high":
                    await self.dao.update_ticket_priority(
                        ticket_info["ticket_id"], "high", message.author.id
                    )
    ...
    async def _auto_assign_responding_staff(
        ...
        try:
            success = await self.dao.assign_ticket(
                ticket_info["ticket_id"], message.author.id, "auto_system"
            )
    ...
    async def _flag_negative_sentiment(
        self, message: discord.Message, ticket_info: Dict, score: int
    ):
        """標記負面情感"""
        try:
            # 取得設定
            ...
            if log_channel_id:
                ...
                if log_channel:
                    ...
                    embed.add_field(
                        name="建議", value="建議主管或資深客服介入處理", inline=False
                    )
    ...
    async def _detect_repetitive_issues(
        ...
        if len(messages_history) >= 3:
            ...
            similarity_count = sum(
                ...
                if self._calculate_similarity(message.content, msg) > 0.7
            )
    ...
    async def on_guild_channel_delete(self, channel: discord.TextChannel):
        ...
        try:
            ...
            if ticket_info and ticket_info["status"] == "open":
                # 自動關閉票券記錄
                await self.dao.close_ticket(channel.id, "system", "頻道被刪除")
    ...
    async def _log_channel_deletion(
        ...
        try:
            ...
            embed.add_field(
                name="票券編號", value=f"#{ticket_info['ticket_id']:04d}", inline=True
            )
            embed.add_field(name="類型", value=ticket_info["type"], inline=True)
            embed.add_field(
                name="開票者", value=f"<@{ticket_info['discord_id']}>", inline=True
            )
            ...
            logger.error(f"記錄頻道刪除事件時發生錯誤: {e}")
    ...
    async def _handle_staff_role_removed(self, member: discord.Member):
        ...
        try:
            ...
            logger.error(f"處理客服角色移除時發生錯誤: {e}")
            ...
            async with self.dao.db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if cursor.rowcount > 0:
                        logger.info(f"清理了 {cursor.rowcount} 個統計快取項目")

        except Exception as e:
            logger.error(f"清理統計快取時發生錯誤: {e}")
        ...
        try:
            ...
            async with self.dao.db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if cursor.rowcount > 0:
                        logger.info(f"清理了 {cursor.rowcount} 個自動回覆日誌")

        except Exception as e:
            logger.error(f"清理自動回覆日誌時發生錯誤: {e}")
        ...
        try:
            ...
            for guild in self.bot.guilds:
                try:
                    ...
                    logger.error(f"清理統計快取項目時發生錯誤: {e}")
