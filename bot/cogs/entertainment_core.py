...
from bot.views.entertainment_views import (
    EntertainmentMenuView, GuessNumberView, RockPaperScissorsView,
    CoinFlipView, DiceRollView, TruthDareView, QuizView,
    GameLeaderboardView, MiniGameView
)
...
class EntertainmentCore(commands.Cog):
    ...
    @app_commands.command(name="entertainment", description="🎮 開啟娛樂中心 - 各種小遊戲等你來玩！")
    async def entertainment_center(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 添加每日限制信息
            remaining = self.game_config["daily_limit"] - self.daily_limits.get(interaction.user.id, 0)
            embed.add_field(
                name="⏰ 今日剩餘",
    ...
    async def game_stats(self, interaction: discord.Interaction, user: Optional[discord.Member] = None):
        """查看遊戲統計"""
