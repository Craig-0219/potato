...
class AutoUpdateManager:
    ...
    async def check_for_updates(self) -> Dict:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                # 獲取最新 release
                url = (
                    f"{self.github_api_base}/repos/{self.repo_owner}/"
                    f"{self.repo_name}/releases/latest"
                )
    ...
    async def perform_update(self, force: bool = False) -> Dict:
        ...
        if not force and not self._is_maintenance_window():
            ...
            return {"error": f"非維護窗口時間，下次維護窗口: {next_window}"}
        ...
        try:
            ...
            return {"success": True, "message": "更新完成，Bot 即將重啟"}
    ...
    async def _execute_git_update(self) -> Dict:
        ...
        try:
            # 1. Stash 任何本地更改
            subprocess.run(["git", "stash"], capture_output=True, text=True, timeout=60)
            ...
            if pull_result.returncode == 0:
                return {"success": True, "output": pull_result.stdout}
            else:
    ...
    def _is_maintenance_window(self) -> bool:
        ...
        if self.maintenance_window_start <= self.maintenance_window_end:
            ...
            # 跨天的維護窗口
            return (
                current_hour >= self.maintenance_window_start
                or current_hour < self.maintenance_window_end
            )
    ...
    def _get_next_maintenance_window(self) -> str:
        ...
        next_window = now.replace(
            hour=self.maintenance_window_start, minute=0, second=0, microsecond=0
        )
...
class AutoUpdateCog(commands.Cog):
    ...
    async def update_status(self, ctx):
        ...
        embed = discord.Embed(title="📊 自動更新器狀態", color=discord.Color.blue())
