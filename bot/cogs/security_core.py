...
class SecurityCore(commands.Cog):
        ...
        try:
            ...
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能查看安全儀表板", ephemeral=True
                )
                return
    ...
    async def security_events(
        ...
        try:
            ...
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message(
                    "❌ 需要管理伺服器權限才能查看安全事件", ephemeral=True
                )
                return
    ...
    async def security_alerts(
        ...
        try:
            ...
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message(
                    "❌ 需要管理伺服器權限才能查看安全警報", ephemeral=True
                )
                return
    ...
    @app_commands.describe(standard="合規標準", days="報告期間天數")
    @app_commands.choices(
        standard=[
    ...
    async def compliance_report(
        self, interaction: discord.Interaction, standard: str, days: int = 90
    ):
        """生成合規報告"""
        try:
            # 檢查權限
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能生成合規報告", ephemeral=True
                )
                return

            if not 30 <= days <= 365:
                await interaction.response.send_message(
                    "❌ 報告期間必須在30-365天之間", ephemeral=True
                )
                return
            ...
            embed.set_footer(
                text=f"報告ID: {report.id} | 生成時間: {report.generated_at.strftime('%Y-%m-%d %H:%M')}"
            )
    ...
    async def compliance_history(
        ...
        try:
            ...
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message(
                    "❌ 需要管理伺服器權限才能查看合規歷史", ephemeral=True
                )
                return
            ...
            reports = await self.security_dao.get_compliance_reports(
                interaction.guild.id, query_standard
            )
