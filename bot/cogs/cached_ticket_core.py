...
# 快取和監控
from shared.cache_manager import cache_manager, cached, CacheStrategy
...
class CachedTicketCore(commands.Cog):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                self.performance_stats['avg_response_time'] = (
                    (current_avg * (total_commands - 1) + duration) / total_commands
                )
    ...
    async def cache_control(self, interaction: discord.Interaction, action: str, target: str = "all"):
        """快取控制命令"""
        try:
            await interaction.response.defer(ephemeral=True)
