...
class TicketCore(commands.Cog):
    ...
    async def _is_ticket_channel(self, channel: discord.TextChannel) -> bool:
        ...
        try:
            ...
            return hasattr(channel, "name") and channel.name.startswith(
                "ticket-"
            )
    ...
    async def setup_ticket(self, ctx: commands.Context):
        ...
        try:
            ...
            await self.DAO.save_panel_message(
                ctx.guild.id, message.id, ctx.channel.id
            )
        ...
        *,
        ...
        try:
            ...
            await self.DAO.update_settings(
                ctx.guild.id, {"category_id": category.id}
            )
    ...
    @commands.command(
        name="ticket_settings", aliases=["settings", "ticket_config"]
    )
    @commands.has_permissions(manage_guild=True)
        ...
        try:
            ...
            if category_id:
                ...
                category_info = (
                    category.mention
                    if category
                    else f"<#{category_id}> (已刪除)"
                )
            else:
                category_info = "❌ 尚未設定"

            embed.add_field(
                name="📁 票券分類", value=category_info, inline=False
            )
            ...
            embed.add_field(name="👥 客服角色", value=role_info, inline=False)
    ...
    @commands.has_permissions(manage_guild=True)
    async def assign_ticket_command(
        self, ctx: commands.Context, ticket_id: int, member: discord.Member
    ):
        """
        手動指派票券給客服人員
    ...
    async def staff_workload_command(
        ...
        try:
            if member:
                ...
                # 查看所有客服的工作量摘要
                summary = (
                    await self.assignment_manager.get_staff_workload_summary(
                        ctx.guild.id
                    )
                )
                ...
                for i, staff in enumerate(summary[:10], 1):
                    ...
                    status_emoji = (
                        "🟢"
                        if staff["load_status"] == "輕鬆"
                        else "🟡" if staff["load_status"] == "適中" else "🔴"
                    )
                ...
                if len(summary) > 10:
                    embed.set_footer(
                        text=f"顯示前10位，共{len(summary)}位客服"
                    )
    ...
    @commands.has_permissions(manage_guild=True)
    async def add_specialty_command(
        self,
        ctx: commands.Context,
        ...
        try:
            success, message = (
                await self.assignment_manager.add_staff_specialty(
    ...
    @commands.has_permissions(manage_guild=True)
    async def assignment_stats_command(
    ...
    async def set_priority(
        ...
        try:
            ...
            if ticket_id:
                ...
                if not await self._is_ticket_channel(interaction.channel):
                    await interaction.response.send_message(
                        "❌ 請在票券頻道中使用，或指定票券ID。", ephemeral=True
                    )
                    return
                ticket = await self.DAO.get_ticket_by_channel(
            ...
            if not (
                is_staff or interaction.user.guild_permissions.manage_guild
            ):
                await interaction.response.send_message(
                    "❌ 只有客服人員或管理員可以設定優先級。", ephemeral=True
                )
                return

            # 更新優先級
            success = await self.DAO.update_ticket_priority(
                ticket["id"], priority
            )
            ...
            if success:
                ...
                # 根據優先級設定顏色
                embed.color = TicketConstants.PRIORITY_COLORS.get(
                    priority, 0xFFAA00
                )
                ...
                # 如果是高優先級，通知管理員
                if priority == "high":
                    await self._notify_high_priority_ticket(
            ...
            await interaction.response.send_message(
                "❌ 設定過程中發生錯誤，請稍後再試。", ephemeral=True
            )

    @commands.command(name="priority_stats", aliases=["pstats"])
    @commands.has_permissions(manage_guild=True)
    async def priority_stats_command(
        self, ctx: commands.Context, days: int = 7
    ):
        """
        查看優先級統計
    ...
    async def _get_priority_statistics(
        ...
        try:
            ...
            # 篩選時間範圍內的票券
            filtered_tickets = [
                t for t in tickets if t["created_at"] >= start_date
            ]
            ...
            for priority in completion_rate:
                if distribution[priority] > 0:
                    completion_rate[priority] = (
                        completion_count[priority] / distribution[priority]
                    ) * 100
                else:
    ...
    async def _notify_high_priority_ticket(
        self, guild: discord.Guild, ticket: Dict, user: discord.Member
    ):
        """通知高優先級票券"""
        try:
    ...
    async def close_ticket(
        ...
        try:
            ...
            try:
                ...
                # 批量記錄頻道歷史訊息
                message_count = (
                    await transcript_manager.batch_record_channel_history(
            ...
            if success:
                ...
                if ticket.get("assigned_to"):
                    await self.assignment_manager.update_ticket_completion(
                        ticket["id"]
                    )
                embed = EmbedBuilder.build(
                    title="✅ 票券已關閉",
                ...
                if reason:
                    embed.add_field(
                        name="關閉原因", value=reason, inline=False
                    )
                await interaction.response.send_message(embed=embed)
                # 評分（僅創建者可評分，未評過才顯示）
                if (
                    str(interaction.user.id) == ticket["discord_id"]
                    and request_rating
                    and not ticket.get("rating")
                ):
                    await asyncio.sleep(2)
                    await self._show_rating_interface(
                        interaction.channel, ticket["id"]
                    )
                await self._schedule_channel_deletion(
                    interaction.channel, delay=30
                )
            else:
                await interaction.response.send_message(
    ...
    async def ticket_info(
        ...
        try:
            ...
            if not can_view:
                ...
                return
            embed = await self._build_ticket_info_embed(
                ticket, interaction.guild
            )
            await interaction.response.send_message(
    ...
    async def test_tickets_disabled(self, interaction: discord.Interaction):
        ...
        try:
            logger.info(
                f"🧪 tickets_test 指令被調用 - 用戶: {interaction.user}"
            )
            await interaction.response.send_message(
    ...
    @app_commands.command(
        name="tickets", description="查看票券列表 | View ticket list"
    )
    @app_commands.describe(
        status="狀態篩選",
        user="指定用戶（客服限定）",
    ...
    async def list_tickets(
        ...
        try:
            # 添加基本測試回應
            logger.info(
                f"🧪 /tickets 指令被調用 - 用戶: {interaction.user}, 伺服器: {interaction.guild.name if interaction.guild else 'DM'}"
            )
            ...
            if tag:
                # 先查找標籤
                tags = await self.tag_dao.get_tags_by_guild(
                ...
                # 篩選出有指定標籤的票券
                filtered_tickets = [
                    t for t in tickets if t["id"] in tagged_ticket_ids
                ]
    ...
    async def tag_group(self, ctx: commands.Context):
        ...
        if ctx.invoked_subcommand is None:
            # 顯示標籤列表
            formatted_list = await self.tag_manager.get_formatted_tag_list(
                ctx.guild.id
            )
    ...
    async def create_tag(
        self, ctx: commands.Context, name: str, *, display_name: str
    ):
        """
        創建新標籤
    ...
    async def init_default_tags(self, ctx: commands.Context):
        ...
        try:
            success, message, count = (
                await self.tag_manager.initialize_default_tags(
    ...
    async def add_tag_to_ticket_slash(
        ...
        try:
            ...
            if not (
                ...
                await interaction.response.send_message(
                    "❌ 只有客服人員或管理員可以添加標籤。", ephemeral=True
                )
                return
            ...
            if not tag:
                await interaction.response.send_message(
                    f"❌ 找不到標籤 '{tag_name}'", ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                "❌ 添加標籤時發生錯誤，請稍後再試。", ephemeral=True
            )
    ...
    async def dashboard_command(self, ctx: commands.Context, days: int = 30):
        ...
        try:
            ...
            # 取得統計數據
            dashboard_data = (
                await self.statistics_manager.get_dashboard_statistics(
                    ctx.guild.id, days
                )
            )
            ...
            if not dashboard_data:
                await loading_msg.edit(
                    content="❌ 無法生成統計數據，請稍後再試。"
                )
                return

            # 建立統計面板嵌入
            embed = await self._build_dashboard_embed(
                dashboard_data, ctx.guild
            )
    ...
    async def realtime_stats(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not (
                ...
                await interaction.response.send_message(
                    "❌ 只有客服人員或管理員可以查看統計。", ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                "❌ 查看統計時發生錯誤，請稍後再試。", ephemeral=True
            )
    ...
    async def _build_dashboard_embed(
        self, dashboard_data: Dict[str, Any], guild: discord.Guild
    ) -> discord.Embed:
        """建立統計面板嵌入"""
        try:
            embed = EmbedBuilder.build(
                ...
                for priority, emoji in [
                    ...
                    if stats:
                        priority_text.append(
                            f"{emoji} {stats.get('count', 0)}張 ({stats.get('completion_rate', 0)}%)"
                        )
            ...
            if trends and trends.get("changes"):
                ...
                created_emoji = (
                    "📈"
                    if changes.get("created_change", 0) > 0
                    else "📉" if changes.get("created_change", 0) < 0 else "➡️"
                )
                closed_emoji = (
                    "📈"
                    if changes.get("closed_change", 0) > 0
                    else "📉" if changes.get("closed_change", 0) < 0 else "➡️"
                )
        ...
        try:
            ...
            for guild_id, tickets in guild_tickets.items():
                try:
                    ...
                    if guild:
                        await self._handle_guild_overdue_tickets(
                            guild, tickets
                        )
                except Exception as e:
            ...
            for guild in self.bot.guilds:
                try:
                    ...
                    if auto_close_hours > 0:
                        ...
                        if closed_count > 0:
                            logger.info(
                                f"自動關閉了 {closed_count} 張無活動票券 - 伺服器: {guild.name}"
                            )
                except Exception as e:
    ...
    async def _check_close_permission(
        self, user: discord.Member, ticket: Dict, settings: Dict
    ) -> bool:
        """
        票券創建者或客服可關閉
        """
    ...
    async def _build_ticket_info_embed(
        ...
        status_emoji = TicketConstants.STATUS_EMOJIS.get(
            ticket["status"], "🟢"
        )
        ...
        embed = EmbedBuilder.build(
            title=f"🎫 票券 #{ticket['id']:04d}", color=color
        )
        embed.add_field(
            name="📋 基本資訊",
            value=f"**類型：** {ticket['type']}\n"
        ...
        if ticket.get("rating"):
            ...
            if ticket.get("rating_feedback"):
                feedback = (
                    ticket["rating_feedback"][:100] + "..."
                    if len(ticket["rating_feedback"]) > 100
                    else ticket["rating_feedback"]
                )
                rating_text += f"\n**回饋：** {feedback}"
    ...
    async def _build_tickets_list_embed(
        self,
        tickets: List[Dict],
        total: int,
        status: str,
        ...
        # 分組顯示票券（按優先級）
        high_priority = [
            t for t in sorted_tickets if t.get("priority") == "high"
        ]
        medium_priority = [
            t
            for t in sorted_tickets
            if t.get("priority", "medium") == "medium"
        ]
        low_priority = [
            t for t in sorted_tickets if t.get("priority") == "low"
        ]
        ...
        for priority_group, group_name, group_emoji in [
            ...
            for ticket in priority_group:
                ...
                field_value = (
                    f"{priority_emoji} **{group_name}** | {status_text}\n"
                )
                ...
                if ticket["status"] == "open":
                    time_info = (
                        f"📅 建立於 {get_time_ago(ticket['created_at'])}"
                    )
                    # 添加 SLA 狀態提示
                ...
                # 票券標題加上優先級標識
                ticket_title = (
                    f"#{ticket['id']:04d} {priority_emoji} {ticket['type']}"
                )

                embed.add_field(
                    name=ticket_title, value=field_value, inline=True
                )
        ...
        try:
            ...
            for priority, priority_tickets in priority_groups.items():
                ...
                for ticket in priority_tickets[:5]:
                    overdue_time = self._calculate_overdue_time(
                        ticket, settings
                    )
                    ticket_list.append(
        ...
        try:
            ...
            for ticket in inactive_tickets:
                try:
                    ...
                    logger.error(
                        f"自動關閉票券 #{ticket['ticket_id']:04d} 錯誤: {e}"
                    )
                    continue
            return closed_count
        except Exception as e:
...
async def setup(bot: commands.Bot):
    ...
    logger.info(
        f"✅ 票券核心系統已載入，註冊的指令: {', '.join(commands_registered)}"
    )
