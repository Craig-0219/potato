...
class ContentAnalysisCog(commands.Cog):
        ...
        try:
            ...
            if result.success and result.toxicity:
                ...
                if toxicity.is_toxic:
                    ...
                    status_text = "檢測到有害內容" if toxicity.toxicity_score > 0.7 else "檢測到潛在問題"
                else:
                    color = 0x00FF00
                    status_emoji = "✅"
                    status_text = "內容安全"
    ...
    async def comprehensive_analysis(self, interaction: discord.Interaction, text: str):
        ...
        try:
            ...
            result = await content_analyzer.analyze_content(
                ...
                [AnalysisType.SENTIMENT, AnalysisType.TOXICITY, 
                 AnalysisType.LANGUAGE, AnalysisType.KEYWORDS, AnalysisType.LINK_SAFETY]
            )
    ...
    async def content_statistics(self, interaction: discord.Interaction, days: int = 7):
        ...
        try:
            ...
            await interaction.followup.send("❌ 獲取內容統計時發生錯誤，請稍後再試。", ephemeral=True)
