...
from bot.services.data_management_service import (
    data_management_service, DataExportRequest, ExportFormat, DataRetentionPolicy
)
from bot.services.guild_analytics_service import guild_analytics_service
from bot.services.guild_permission_manager import guild_permission_manager, GuildPermission
...
class GuildManagementCore(commands.Cog):
    ...
    @app_commands.command(name="export_data", description="📤 導出伺服器數據 (GDPR)")
    ...
    async def _process_data_export(self, interaction: discord.Interaction, 
                                  export_request: DataExportRequest):
        """處理數據導出"""
        try:
            # 執行導出
            export_data = await self.data_service.export_guild_data(export_request)
    ...
    @app_commands.command(name="guild_analytics", description="📊 伺服器分析儀表板")
    @app_commands.describe(days="分析天數 (1-30)")
    @app_commands.default_permissions(administrator=True)
    async def guild_analytics(self, interaction: discord.Interaction, days: int = 7):
        """顯示伺服器分析儀表板"""
        try:
            if not await SafeInteractionHandler.safe_defer(interaction, ephemeral=True):
                return
            ...
            # 獲取分析數據
            dashboard_data = await self.analytics_service.get_guild_analytics_dashboard(guild_id, days)
            ...
            if performance:
                ...
                embed.add_field(name="⚡ 性能指標", value=perf_text, inline=True)
    ...
    @app_commands.command(name="manage_permissions", description="👥 管理用戶權限")
    @app_commands.describe(
        user="目標用戶",
        action="操作類型",
        role="角色名稱"
    ...
    @app_commands.default_permissions(administrator=True)
    async def manage_permissions(self, interaction: discord.Interaction,
                               user: discord.Member,
                               action: str,
                               role: str = None):
        """管理用戶權限"""
        try:
            ...
            if action == "view":
                ...
                # 角色資訊
                roles_text = ", ".join([role.value for role in user_perms.roles])
                embed.add_field(name="🎭 角色", value=roles_text or "無", inline=False)
