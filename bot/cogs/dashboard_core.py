...
class DashboardCore(commands.Cog):
    ...
    @app_commands.command(
        name="dashboard_overview", description="查看系統概覽儀表板"
    )
        ...
        try:
            ...
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message(
                    "❌ 需要管理伺服器權限才能查看儀表板", ephemeral=True
                )
                return
            ...
            if not 1 <= days <= 365:
                await interaction.response.send_message(
                    "❌ 分析天數必須在1-365天之間", ephemeral=True
                )
                return
            ...
            if refresh:
                await self.dashboard_manager.clear_dashboard_cache(
                    f"overview_{interaction.guild.id}_{days}"
                )

            # 生成儀表板數據
            dashboard_data = (
                await self.dashboard_manager.generate_overview_dashboard(
            ...
            for metric_name, metric_data in dashboard_data.metrics.items():
                emoji = (
                    ...
                    else "📉" if metric_data.trend == "down" else "➡️"
                )
                status_emoji = (
                    "🟢"
                    if metric_data.status == "good"
                    else "🟡" if metric_data.status == "warning" else "🔴"
                )
    ...
    async def dashboard_performance(
        ...
        try:
            ...
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message(
                    "❌ 需要管理伺服器權限才能查看性能儀表板", ephemeral=True
                )
                return
            ...
            for metric_name, metric_data in dashboard_data.metrics.items():
                if metric_name in [
                    ...
                    status_icon = (
                        "🟢" if metric_data.status == "good" else "🟡"
                    )
            ...
            embed.set_footer(
                text=f"分析期間: {days}天 | 生成時間: {dashboard_data.generated_at.strftime('%H:%M:%S')}"
            )
    ...
    async def dashboard_prediction(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能查看預測分析", ephemeral=True
                )
                return
            ...
            # 生成預測儀表板
            dashboard_data = (
                await self.dashboard_manager.generate_predictive_dashboard(
            ...
            for metric_name, metric_data in dashboard_data.metrics.items():
                ...
                prediction_summary.append(
                    f"{trend_emoji} {metric_name.replace('_', ' ').title()}: {metric_data.current_value}"
                )
    ...
    @app_commands.describe(action="操作類型", cache_key="特定快取鍵 (可選)")
    ...
    async def dashboard_cache(
        ...
        try:
            ...
            if action == "info":
                # 查看快取資訊
                cache_info = (
                    await self.dashboard_manager.get_dashboard_cache_info()
                )

                embed = EmbedBuilder.build(
                    title="🗄️ 儀表板快取資訊", color=0x95A5A6
                )
                ...
                if cache_info["cache_keys"]:
                    embed.add_field(
                        ...
                        value="\n".join(
                            f"• `{key}`"
                            for key in cache_info["cache_keys"][:5]
                        ),
                        inline=False,
                    )
    ...
    @app_commands.command(
        name="dashboard_realtime", description="查看實時系統狀態"
    )
    ...
    async def _get_realtime_data(self, guild_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            realtime_stats = await self.stats_manager.get_realtime_stats(
                guild_id
            )
            ...
            try:
                # 獲取工作流程數據（如果可用）
                workflow_data = await self._get_workflow_realtime_data(
                    guild_id
                )
            except Exception as e:
    ...
    async def _get_workflow_realtime_data(
        self, guild_id: int
    ) -> Dict[str, Any]:
        """獲取工作流程實時數據"""
        try:
            ...
            # 獲取活躍工作流程
            active_workflows = await workflow_dao.get_active_workflows(
                guild_id
            )
            ...
            today_executions = await workflow_dao.get_executions_count(
                guild_id=guild_id, start_time=today_start
            )

            # 獲取執行中的工作流程
            running_executions = (
                await workflow_dao.get_running_executions_count(guild_id)
            )

            return {
                "active_workflows": (
                    len(active_workflows) if active_workflows else 0
                ),
                "running_executions": (
                    running_executions if running_executions else 0
                ),
                "today_executions": (
                    today_executions if today_executions else 0
                ),
            }
