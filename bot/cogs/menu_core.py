...
# 匯入選單系統
from bot.ui.menu_system import (
    MenuSystemManager, MainMenuView, AdminMenuView,
    MenuType, MenuStyle
)
...
class MenuCore(commands.Cog):
    ...
    @app_commands.command(name="admin_gui", description="👑 管理員控制面板 - 伺服器管理專用GUI (需要管理員權限)")
...
class QuickMenuView(discord.ui.View):
    ...
    async def quick_create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        """快速建立票券"""
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 只有選單發起者可以使用", ephemeral=True)
            return
