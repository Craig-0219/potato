...
class MinecraftCore(commands.Cog):
    ...
    async def minecraft_status(self, ctx):
        ...
        try:
            ...
            # 建立狀態嵌入訊息
            embed = discord.Embed(
                title="🟢 Minecraft 伺服器狀態" if status['online'] else "🔴 Minecraft 伺服器狀態",
    ...
    async def minecraft_command(self, ctx, command: str):
        ...
        try:
            ...
            if result['success']:
                embed.add_field(
                    name="✅ 執行結果",
                    value=f"```\n{result['response'][:1000]}\n```" if result['response'] else "指令執行成功",
                    inline=False
                )
            else:
                embed.add_field(
    ...
    @commands.has_permissions(administrator=True)
    async def whitelist_management(self, ctx, action: discord.Option(str, choices=["add", "remove", "list"]), 
                                 player: str = None):
        """管理 Minecraft 伺服器白名單"""
        try:
            await ctx.defer()
            ...
            if action == "add":
                ...
                title = f"✅ 已將 {player} 加入白名單" if result['success'] else f"❌ 無法將 {player} 加入白名單"
            elif action == "remove":
                result = await self.rcon_manager.execute_command(f"whitelist remove {player}")
                title = f"✅ 已將 {player} 從白名單移除" if result['success'] else f"❌ 無法將 {player} 從白名單移除"
    ...
    @commands.slash_command(name="mc-chat-bridge", description="設置 Discord ↔ Minecraft 聊天橋接 (需要管理員權限)")
    @commands.has_permissions(administrator=True)
    async def chat_bridge_control(self, ctx, action: discord.Option(str, choices=["enable", "disable", "status"])):
        """控制聊天橋接功能"""
        try:
            await ctx.defer()
            ...
            if action == "enable":
                ...
                if result:
                    embed.description = f"已在 {ctx.channel.mention} 啟用 Discord ↔ Minecraft 聊天橋接"
                    embed.add_field(
                        name="使用說明",
                        value="""
                ...
                if status['enabled'] and status['discord_channel_id']:
                    ...
                    embed.add_field(
                        name="橋接頻道",
                        value=channel.mention if channel else f"ID: {status['discord_channel_id']}",
                        inline=True
                    )
