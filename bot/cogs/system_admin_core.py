...
class SystemAdmin(commands.Cog):
    ...
    @app_commands.command(
        name="basic_dashboard", description="查看基礎系統儀表板"
    )
        ...
        try:
            ...
            for btype in backup_types:
                try:
                    ...
                    export_result = await export_manager.export_data(
                        export_request
                    )
                    ...
                    results[btype] = {"success": False, "error": str(e)}
    ...
    @app_commands.describe(action="管理動作", target="目標對象")
    @app_commands.choices(
        action=[
        ...
        try:
            ...
            if action == "health":
                ...
                try:
                    ...
                    # 投票統計
                    vote_stats = stats.get("vote_statistics", {}).get(
                        "summary", {}
                    )
                    embed.add_field(
                        name="🗳️ 投票系統",
                    ...
                    # 系統統計
                    system_stats = stats.get("system_statistics", {}).get(
                        "summary", {}
                    )
                    embed.add_field(
                        name="⚙️ 系統狀態",
            ...
            embed.set_footer(
                text=f"由 {interaction.user.display_name} 執行 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            await interaction.edit_original_response(embed=embed)
