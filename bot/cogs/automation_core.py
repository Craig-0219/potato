...
from bot.services.automation_engine import automation_engine, TriggerType, ActionType, ConditionOperator, RuleStatus
...
class AutomationCore(commands.Cog):
    ...
    async def automation_list(
        ...
        try:
            ...
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message("❌ 需要管理伺服器權限才能查看自動化規則", ephemeral=True)
                return
    ...
    async def automation_create(self, interaction: discord.Interaction, name: str, description: str = ""):
        """創建新的自動化規則"""
        try:
            # 檢查權限
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message("❌ 需要管理伺服器權限才能創建自動化規則", ephemeral=True)
                return
    ...
    async def automation_detail(self, interaction: discord.Interaction, rule_id: str):
        ...
        try:
            # 檢查權限
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message("❌ 需要管理伺服器權限才能查看規則詳情", ephemeral=True)
                return
            ...
            # 基本資訊
            status_emoji = {'active': '🟢', 'draft': '⚪', 'paused': '🟡', 'disabled': '🔴', 'error': '❌'}.get(rule['status'], '⚪')
            status_name = {'active': '啟用', 'draft': '草稿', 'paused': '暫停', 'disabled': '停用', 'error': '錯誤'}.get(rule['status'], rule['status'])
    ...
    async def automation_toggle(self, interaction: discord.Interaction, rule_id: str, status: str):
        ...
        try:
            # 檢查權限
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message("❌ 需要管理伺服器權限才能管理規則狀態", ephemeral=True)
                return
    ...
    async def automation_history(
        ...
        try:
            # 檢查權限
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message("❌ 需要管理伺服器權限才能查看執行記錄", ephemeral=True)
                return
    ...
    async def automation_stats(self, interaction: discord.Interaction, days: int = 30):
        ...
        try:
            # 檢查權限
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message("❌ 需要管理伺服器權限才能查看統計資訊", ephemeral=True)
                return
            ...
            if stats.get('top_rules'):
                ...
                for rule in stats['top_rules'][:5]:
                    ...
                    top_rules_text.append(f"• {rule['name']}: {rule['execution_count']}次 (最後: {last_exec})")
