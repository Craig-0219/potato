...
class CrossPlatformEconomyCore(commands.Cog):
        ...
        try:
            ...
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能設定跨平台同步。", ephemeral=True
                )
                return
            ...
            if enable:
                success = (
                    await self.economy_manager.enable_cross_platform_sync(
    ...
    async def economy_settings(
        ...
        try:
            ...
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能調整經濟設定。", ephemeral=True
                )
                return
            ...
            if daily_coins_base is not None:
                updates["daily_coins_base"] = max(
                    10, min(100, daily_coins_base)
                )
            if daily_coins_max is not None:
                updates["daily_coins_max"] = max(
                    100, min(1000, daily_coins_max)
                )
            ...
            embed.add_field(
                ...
                + (
                    f"\nAPI：`{settings.minecraft_api_endpoint[:30]}...`"
                    if settings.minecraft_api_endpoint
                    else ""
                ),
                inline=False,
            )
            ...
            embed.set_footer(
                text=f"最後更新：{settings.last_updated.strftime('%Y-%m-%d %H:%M:%S')} UTC"
            )
    ...
    async def economy_stats(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = EmbedBuilder.create_info_embed(
                "📊 跨平台經濟統計", f"{interaction.guild.name} 的經濟系統報告"
            )
            ...
            if cross_platform_stats.get("sync_enabled"):
                ...
                platform_dist = cross_platform_stats.get(
            ...
            embed.set_footer(
                text=f"統計時間：{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC"
            )
    ...
    async def force_sync(
        ...
        try:
            ...
            # 獲取用戶經濟數據顯示結果
            economy_data = await self.economy_manager.get_user_economy(
                user.id, interaction.guild.id
            )

            result_embed = EmbedBuilder.create_info_embed(
                "📋 同步結果", f"{user.display_name} 的經濟數據"
            )
    ...
    async def manual_anti_inflation(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能執行抗通膨調整。", ephemeral=True
                )
                return
    ...
    async def handle_minecraft_webhook(
        ...
        try:
            return await self.economy_manager.handle_minecraft_webhook(
                webhook_data
            )
        except Exception as e:
    ...
    async def zientis_user_link(self, interaction: discord.Interaction):
        ...
        try:
            ...
            logger.info(
                f"🔗 用戶 {interaction.user.id} 生成 Minecraft 綁定驗證碼: {verification_code}"
            )
