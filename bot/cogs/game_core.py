...
from bot.views.game_views import (
    GameMenuView, GuessNumberView, RockPaperScissorsView, 
    TriviaView, CoinFlipView, RouletteView
)
...
class GameEntertainment(commands.Cog):
    ...
    async def daily_checkin(self, interaction: discord.Interaction):
        ...
        try:
            ...
            prometheus_metrics.increment_counter(
                'potato_bot_daily_checkins_total',
                {'guild': str(guild_id)}
            )
            
        except Exception as e:
            logger.error(f"❌ 每日簽到錯誤: {e}")
            await interaction.response.send_message("❌ 簽到時發生錯誤，請稍後再試。", ephemeral=True)
    ...
    async def check_balance(self, interaction: discord.Interaction, user: discord.User = None):
        ...
        try:
            ...
            embed = EmbedBuilder.build(
                title=f"💰 {target_user.display_name} 的錢包",
                color=0xFFD700
            )
    ...
    async def leaderboard(self, interaction: discord.Interaction, category: str = "coins"):
        ...
        try:
            ...
            # 獲取排行榜數據
            leaderboard_data = await self.economy_manager.get_leaderboard(guild_id, category, limit=10)
            ...
            if not leaderboard_data:
                ...
                for i, entry in enumerate(leaderboard_data[:10], 1):
                    ...
                    rank_text.append(f"{rank_emoji} {username}\n{category_emojis[category]} {value_text}")
            ...
            embed.set_footer(text=f"數據更新時間: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
    ...
    async def view_achievements(self, interaction: discord.Interaction, user: discord.User = None):
        ...
        try:
            ...
            # 獲取用戶成就
            user_achievements = await self.achievement_manager.get_user_achievements(user_id, guild_id)
                ...
                for rarity in rarity_order:
                    if rarity in rarity_groups:
                        ...
                        if len(rarity_groups[rarity]) > 5:
                            achievements_list.append(f"... 還有 {len(rarity_groups[rarity]) - 5} 個")
    ...
    async def achievement_progress(self, interaction: discord.Interaction, achievement_id: str = None):
        """查看成就進度"""
        try:
            user_id = interaction.user.id
            guild_id = interaction.guild.id
    ...
    async def link_minecraft_account(self, interaction: discord.Interaction, minecraft_username: str):
        """綁定Minecraft帳號"""
        try:
            ...
            await interaction.response.send_message("❌ 綁定過程中發生錯誤，請稍後再試。", ephemeral=True)
    ...
    async def unlink_minecraft_account(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("❌ 解綁過程中發生錯誤，請稍後再試。", ephemeral=True)
    ...
    async def sync_economy_data(self, interaction: discord.Interaction, direction: str = "to_minecraft"):
        """手動同步經濟數據"""
        try:
    ...
    async def cross_platform_status(self, interaction: discord.Interaction):
        ...
        try:
            # 獲取綁定信息
            link_info = await cross_platform_economy._get_account_link(discord_id=interaction.user.id)
            ...
            if link_info:
                ...
                if transactions:
                    ...
                    embed.add_field(
                        name="📋 最近交易",
                        value="\n".join(recent_transactions) if recent_transactions else "暫無交易記錄",
                        inline=False
                    )
    ...
    async def guess_number_game(self, interaction: discord.Interaction, difficulty: str = "medium"):
        ...
        try:
            # 檢查用戶是否已有活躍遊戲
            user_session = await self._get_user_active_session(interaction.user.id, interaction.guild.id)
            if user_session:
    ...
    async def _get_user_active_session(self, user_id: int, guild_id: int) -> Optional[GameSession]:
        ...
        for session in self.active_sessions.values():
            if (session.player_id == user_id and 
                session.guild_id == guild_id and 
