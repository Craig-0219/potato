...
class WebhookCore(commands.Cog):
        ...
        try:
            ...
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能創建Webhook", ephemeral=True
                )
                return
    ...
    async def config_webhook(
        self, interaction: discord.Interaction, webhook_name: str
    ):
        """配置Webhook"""
        try:
            # 檢查權限
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能配置Webhook", ephemeral=True
                )
                return
    ...
    async def test_webhook(
        ...
        try:
            ...
            await self.webhook_manager.trigger_webhook_event(
                WebhookEvent.CUSTOM_EVENT, interaction.guild.id, test_data
            )
            ...
            if webhook_name:
                ...
                if target_webhook.get("last_triggered"):
                    last_triggered = datetime.fromisoformat(
                        target_webhook["last_triggered"].replace("Z", "+00:00")
                    )
                    embed.add_field(
                        name="⏰ 最後觸發",
    ...
    async def delete_webhook(
        ...
        try:
            ...
            if not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message(
                    "❌ 需要管理員權限才能刪除Webhook", ephemeral=True
                )
                return
            ...
            class ConfirmDeleteView(discord.ui.View):
                ...
                @discord.ui.button(
                    label="確認刪除", style=discord.ButtonStyle.danger
                )
            ...
            view = ConfirmDeleteView(
                target_webhook["id"], target_webhook["name"]
            )
            await interaction.response.send_message(
                embed=embed, view=view, ephemeral=True
            )
