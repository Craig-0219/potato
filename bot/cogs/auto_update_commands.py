...
class AutoUpdateCommands(commands.Cog):
    ...
    async def check_updates(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if update_info.get("has_update"):
                ...
                embed.add_field(
                    name="檢查時間", value=f"<t:{int(datetime.now().timestamp())}:R>", inline=False
                )
    ...
    async def update_info(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title="🔄 系統更新資訊", color=discord.Color.blue(), timestamp=datetime.now()
            )
            ...
            if summary.get("update_available"):
                ...
                if summary["release_notes"]:
                    embed.add_field(
                        name="📝 更新說明", value=summary["release_notes"], inline=False
                    )
                ...
                embed.add_field(
                    name="✅ 狀態", value=summary.get("message", "系統已是最新版本"), inline=False
                )
    ...
    async def _create_update_embed(self, update_info: dict) -> discord.Embed:
        ...
        update_type = (
            "🔴 主要更新"
            if comparison.get("is_major_update")
            else "🟡 次要更新" if comparison.get("is_minor_update") else "🟢 修復更新"
        )

        embed.add_field(name="🏷️ 更新類型", value=update_type, inline=True)

        if comparison.get("commits_count", 0) > 0:
            embed.add_field(
                name="📈 變更統計", value=f"{comparison['commits_count']} 個提交", inline=True
            )
        ...
        if release_info.get("body"):
            ...
            embed.add_field(name="📝 發布說明", value=notes, inline=False)
