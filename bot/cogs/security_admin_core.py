...
from bot.views.security_management_views import (
    SecurityDashboardView, MFASetupView, RoleManagementView,
    SecurityAuditView, APIKeyManagementView
)
...
class SecurityAdminCore(commands.Cog):
    ...
    async def mfa_status(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title="🔐 多因素認證狀態",
                color=discord.Color.green() if mfa_status["mfa_enabled"] else discord.Color.orange()
            )
    ...
    async def assign_role(self, interaction: discord.Interaction, 
                         user: discord.Member, role_name: str, expires_days: Optional[int] = None):
        """分配角色給用戶"""
        try:
            if not await SafeInteractionHandler.safe_defer(interaction, ephemeral=True):
                return
            ...
            # 檢查權限
            if not await rbac_manager.check_permission(assigner_id, guild_id, Permission.USER_MANAGE):
                await SafeInteractionHandler.safe_followup(
                    interaction,
                    "❌ 您沒有分配角色的權限",
                    ephemeral=True
    ...
    @app_commands.default_permissions(administrator=True)
    async def compliance_report(self, interaction: discord.Interaction, 
                              standard: str, days: int = 30):
        """生成合規報告"""
        try:
            if not await SafeInteractionHandler.safe_defer(interaction, ephemeral=True):
                return
    ...
    async def _get_audit_statistics(self, guild_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "critical_threats": len([a for a in suspicious_activities if a.get("severity") == "high"]),
                "high_risk_events": len([a for a in suspicious_activities if a.get("severity") == "medium"]), 
