...
class WelcomeCore(commands.Cog):
    ...
    async def welcome_channel(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        try:
            ...
            success, message = await self.welcome_manager.set_welcome_channel(ctx.guild.id, channel_id)
    ...
    async def leave_channel(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        try:
            channel_id = channel.id if channel else None
            success, message = await self.welcome_manager.set_leave_channel(ctx.guild.id, channel_id)
    ...
    async def autorole_add(self, ctx, role: discord.Role):
        ...
        try:
            ...
            success, message = await self.welcome_manager.set_auto_roles(ctx.guild.id, current_roles)
    ...
    async def autorole_list(self, ctx):
        ...
        try:
            ...
            embed.set_footer(text=f"啟用狀態: {'✅ 已啟用' if settings.get('auto_role_enabled') else '❌ 已停用'}")
    ...
    async def welcome_status(self, ctx):
        ...
        try:
            ...
            # 頻道設定
            welcome_channel = f"<#{settings['welcome_channel_id']}>" if settings.get('welcome_channel_id') else "❌ 未設定"
            leave_channel = f"<#{settings['leave_channel_id']}>" if settings.get('leave_channel_id') else "❌ 未設定"
    ...
    async def welcome_diagnose(self, ctx):
        ...
        try:
            ...
            if not intents.members:
                suggestions.append("🚨 **關鍵問題**：需要在 Discord Developer Portal 啟用 `Server Members Intent`")
