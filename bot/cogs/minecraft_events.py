...
from bot.services.minecraft.event_manager import MinecraftEventManager, EventType, EventStatus
...
class MinecraftEvents(commands.Cog):
    ...
    async def create_event(self, ctx, title: str, event_type: str, description: str, max_participants: int = 0, duration_hours: int = 24, registration_hours: int = 12):
        """建立新活動"""
        try:
            await ctx.defer()
            ...
            if event_id:
                ...
                # 記錄活動建立
                logger.info(f"新活動建立 - ID: {event_id}, 標題: {title}, 主辦人: {ctx.author.display_name}")
    ...
    async def list_events(self, ctx):
        ...
        try:
            ...
            if len(events) > 10:
                embed.set_footer(text=f"顯示 10/{len(events)} 個活動，使用 `/event info <ID>` 查看詳細資訊")
