...
class LocalizedCommand:
    ...
    def command(self, name_key: str, description_key: str, **kwargs):
        ...
        def decorator(func):
            ...
            # 從語言包獲取名稱和描述
            name = self.language_manager.get_string(f"commands.names.{name_key}", default_lang) or name_key
            description = self.language_manager.get_string(f"commands.descriptions.{description_key}", default_lang)
...
class LocalizedResponse:
    ...
    async def get_user_language(self, user_id: int, guild_id: Optional[int] = None) -> str:
        ...
        try:
            ...
            return user_lang_info['language_code'] if user_lang_info else self.language_manager.default_language
        except Exception as e:
            logger.error(f"獲取用戶語言設定錯誤: {e}")
    ...
    async def create_embed(self, interaction: discord.Interaction, title_key: str, 
    ...
    async def send_localized_response(self, interaction: discord.Interaction, 
                                    message_key: str, ephemeral: bool = False, 
                                    embed: bool = True, **kwargs) -> None:
        """發送本地化回應"""
        user_lang = getattr(interaction, 'user_language', self.language_manager.default_language)
        message = self.get_text(message_key, user_lang, **kwargs)
        
        if embed:
            color = 0x28a745 if "success" in message_key else 0xdc3545 if "error" in message_key else 0x3498db
