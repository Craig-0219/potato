...
from bot.utils.ticket_constants import (
    ...
    get_priority_color, get_status_color, calculate_sla_time,
...
class TicketPermissionChecker:
    ...
    @staticmethod
    def can_manage_ticket(user: discord.Member, ticket_info: Dict[str, Any], 
    ...
    @staticmethod
    def can_close_ticket(user: discord.Member, ticket_info: Dict[str, Any], 
                        support_roles: List[int]) -> bool:
        """檢查是否可以關閉票券"""
    ...
    @staticmethod
    def can_view_ticket(user: discord.Member, ticket_info: Dict[str, Any], 
                       support_roles: List[int]) -> bool:
        """檢查是否可以查看票券"""
        if not user or not ticket_info:
            return False
...
def build_ticket_embed(ticket_info: Dict[str, Any], include_stats: bool = False) -> discord.Embed:
    ...
    if ticket_info.get('tags'):
        try:
            tags = json.loads(ticket_info['tags']) if isinstance(ticket_info['tags'], str) else ticket_info['tags']
            if tags:
...
def build_staff_performance_embed(staff_stats: Dict[str, Any], guild: discord.Guild, 
    ...
    # 排序客服（按處理票券數）
    sorted_staff = sorted(staff_stats.items(), 
...
async def send_ticket_notification(user: discord.Member, title: str, description: str, 
                                 color: discord.Color = discord.Color.blue()) -> bool:
    """發送票券通知"""
    try:
        embed = discord.Embed(
            title=title,
