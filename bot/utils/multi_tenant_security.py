...
class MultiTenantSecurityManager:
    ...
    async def validate_guild_access(self, user_id: int, guild_id: int, 
...
class SecureQueryBuilder:
    ...
    def build_select(self, table: str, columns: Union[str, List[str]] = "*", 
                    guild_id: Optional[int] = None, 
                    additional_where: Optional[Dict[str, Any]] = None) -> Tuple[str, List[Any]]:
        """建構安全的 SELECT 查詢"""
    ...
    def build_insert(self, table: str, data: Dict[str, Any], 
                    guild_id: Optional[int] = None) -> Tuple[str, List[Any]]:
        """建構安全的 INSERT 查詢"""
    ...
    def build_delete(self, table: str, where_conditions: Dict[str, Any],
                    guild_id: Optional[int] = None) -> Tuple[str, List[Any]]:
        """建構安全的 DELETE 查詢"""
...
class TenantDataManager:
    ...
    async def delete_guild_data(self, guild_id: int, 
                               retain_logs: bool = True) -> Dict[str, Any]:
        """刪除伺服器資料（GDPR 刪除權）"""
        try:
            deleted_records = {}
            ...
            # 定義需要清理的表格
            tables_to_clean = [
                "tickets", "ticket_settings", "ticket_logs",
                "votes", "vote_responses", "vote_settings",
            ...
            if not retain_logs:
                tables_to_clean.extend([
                    "security_events", "data_access_logs",
