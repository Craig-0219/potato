...
async def sync_economy_data(
    ...
    try:
        ...
        for currency, new_amount in request.balances.items():
            ...
            if current_amount != new_amount:
                ...
                try:
                    if currency == "coins":
                        if difference > 0:
                            await economy_manager.add_coins(request.user_id, request.guild_id, difference)
                        else:
                            await economy_manager.subtract_coins(request.user_id, request.guild_id, abs(difference))
                    elif currency == "gems":
                        await economy_manager.add_gems(request.user_id, request.guild_id, difference)
                    elif currency == "experience":
                        await economy_manager.add_experience(request.user_id, request.guild_id, difference)
        ...
        background_tasks.add_task(
            ...
            request.guild_id, request.sync_type, len(adjustments)
        )
    ...
    try:
        ...
        return PlayerEconomyResponse(
            ...
            sync_status={
                ...
                "pending_syncs": len([t for t in economy_manager._sync_tasks.keys() if t == user_id])
            }
        )
...
async def admin_economy_adjust(
    ...
    try:
        ...
        if action == "anti_inflation":
            ...
            updated_settings = await economy_manager.update_economy_settings(guild_id, **settings_update)
            result = {
                "status": "success",
                "updated_settings": {
