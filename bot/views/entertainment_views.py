...
class RockPaperScissorsView(discord.ui.View):
    ...
    async def play_round(self, interaction: discord.Interaction, user_choice: str, user_emoji: str):
        ...
        # 檢查遊戲是否結束
        if self.games_played >= self.max_games or self.user_score > self.max_games//2 or self.bot_score > self.max_games//2:
            winner = "user" if self.user_score > self.bot_score else "bot" if self.bot_score > self.user_score else "tie"
...
class CoinFlipView(discord.ui.View):
    ...
    async def flip_coin(self, interaction: discord.Interaction, choice: str):
        ...
        if won:
            ...
            embed = EmbedBuilder.create_error_embed(
                f"😅 猜錯了！硬幣是{result_text} {result_emoji}",
                "下次再來！"
            )
...
class QuizView(discord.ui.View):
    ...
    async def answer_question(self, interaction: discord.Interaction, choice: int):
        ...
        for button in self.children:
            if isinstance(button, discord.ui.Button) and button.label.startswith(("A:", "B:", "C:", "D:")):
...
class MiniGameView(discord.ui.View):
    ...
    async def archery_game(self, interaction: discord.Interaction, button: discord.ui.Button):
        # 簡單的射箭遊戲
        target_zones = ["❌ 脫靶", "⚪ 外圈 (1分)", "🔵 中圈 (3分)", "🔴 內圈 (5分)", "🎯 正中 (10分)"]
...
class GameLeaderboardView(discord.ui.View):
    ...
    async def back_to_entertainment(self, interaction: discord.Interaction, button: discord.ui.Button):
