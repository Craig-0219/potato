...
class AutomationView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 只有命令執行者可以操作此介面", ephemeral=True)
            return False
        return True
    ...
    async def view_statistics(self, interaction: discord.Interaction, button: discord.ui.Button):
        """查看統計資訊"""
        await interaction.response.send_message("請使用 `/automation_stats` 指令查看詳細統計", ephemeral=True)
    
    @discord.ui.button(label="📜 執行記錄", style=discord.ButtonStyle.secondary, row=1)
    async def view_history(self, interaction: discord.Interaction, button: discord.ui.Button):
        """查看執行記錄"""
        await interaction.response.send_message("請使用 `/automation_history` 指令查看執行記錄", ephemeral=True)
        ...
        try:
            ...
            for action in actions_data:
                if 'type' not in action or 'parameters' not in action:
                    await interaction.followup.send("❌ 每個動作必須包含 'type' 和 'parameters' 欄位", ephemeral=True)
                    return
        ...
        try:
            ...
            for i, action in enumerate(actions_data):
                if not isinstance(action, dict) or 'type' not in action or 'parameters' not in action:
                    await interaction.response.send_message(f"❌ 動作 {i+1} 格式錯誤，必須包含 'type' 和 'parameters'", ephemeral=True)
                    return
