...
class SecurityView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 只有指令執行者可以操作此介面", ephemeral=True)
            return False
        return True
...
class AlertView(discord.ui.View):
    ...
    async def alert_statistics(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # 狀態分佈
        status_text = []
        status_emojis = {'open': '🔓', 'investigating': '🔍', 'resolved': '✅', 'false_positive': '❌'}
        for status, count in status_counts.items():
            emoji = status_emojis.get(status, '❓')
            percentage = (count / len(self.alerts)) * 100
            status_text.append(f"{emoji} {status.replace('_', ' ').title()}: {count} ({percentage:.1f}%)")
    ...
    async def refresh_alerts(self, interaction: discord.Interaction, button: discord.ui.Button):
        """刷新警報列表"""
        await interaction.response.send_message(
            "請重新使用 `/security_alerts` 指令獲取最新警報",
            ephemeral=True
        )
...
class AlertDetailView(discord.ui.View):
    ...
    async def mark_false_positive(self, interaction: discord.Interaction, button: discord.ui.Button):
        """標記為誤報"""
        await interaction.response.send_message(
...
class AlertResolutionModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(
                name="📝 解決說明",
                value=self.resolution_note.value[:200] + "..." if len(self.resolution_note.value) > 200 else self.resolution_note.value,
                inline=False
            )
            ...
            if self.preventive_actions.value:
                embed.add_field(
                    name="🛡️ 預防措施",
                    value=self.preventive_actions.value[:200] + "..." if len(self.preventive_actions.value) > 200 else self.preventive_actions.value,
                    inline=False
                )
            
            embed.set_footer(text=f"解決時間: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC")
...
class ComplianceReportView(discord.ui.View):
    ...
    async def full_report(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_footer(text=f"生成時間: {self.report.generated_at.strftime('%Y-%m-%d %H:%M:%S')} UTC")
