...
class AIAssistantControlView(discord.ui.View):
    ...
    @discord.ui.button(
        label="🗨️ 開始對話", style=discord.ButtonStyle.primary, emoji="🗨️"
    )
...
class AIModelSettingsView(discord.ui.View):
    ...
    @discord.ui.button(
        label="🔥 提高創意度", style=discord.ButtonStyle.secondary
    )
    ...
    @discord.ui.button(
        label="❄️ 降低創意度", style=discord.ButtonStyle.secondary
    )
    ...
    @discord.ui.button(
        label="📏 增加長度", style=discord.ButtonStyle.secondary
    )
    ...
    @discord.ui.button(
        label="📐 減少長度", style=discord.ButtonStyle.secondary
    )
...
class AIChatModal(discord.ui.Modal, title="🤖 AI 助手對話"):
    ...
    def __init__(self, provider: AIProvider, task_type: AITaskType):
        ...
        self.chat_input.placeholder = placeholders.get(
            task_type, "輸入您的請求..."
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 調用 AI 服務
            logger.info(
                f"🤖 AI 請求: 用戶={interaction.user.name}, 模型={self.provider.value}, 任務={self.task_type.value}"
            )
            ...
            if response.success:
                ...
                embed.set_footer(
                    text=f"任務類型: {self.task_type.value} | 請求 ID: {response.request_id}"
                )

            else:
                # 錯誤回應
                embed = EmbedBuilder.create_error_embed(
                    "❌ AI 回應失敗", f"錯誤信息: {response.error_message}"
                )
