        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"已成功應用以下標籤：\n" + 
                          "\n".join([f"• `{tag['tag_name']}` ({tag['confidence']:.1%})" 
...
class AIPriorityView(discord.ui.View):
    ...
    async def _apply_priority_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"❌ 更新優先級時發生錯誤：{str(e)}", ephemeral=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            try:
                ...
                await interaction.response.send_message("❌ 評分必須是 1-5 之間的數字", ephemeral=True)
                return
            ...
            await interaction.response.send_message(f"❌ 提交回饋時發生錯誤：{str(e)}", ephemeral=True)