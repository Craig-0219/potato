...
from bot.services.security.rbac_manager import rbac_manager, Permission, APIKeyType
...
class APIKeyTypeSelectionView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(label="唯讀密鑰", value="read_only", description="僅限查詢操作", emoji="👁️"),
            discord.SelectOption(label="讀寫密鑰", value="read_write", description="可進行資料修改", emoji="✏️"),
            discord.SelectOption(label="管理員密鑰", value="admin", description="完整管理權限", emoji="👑"),
            discord.SelectOption(label="服務密鑰", value="service", description="系統間通訊", emoji="🔧")
        ]
    )
    async def select_api_key_type(self, interaction: discord.Interaction, select: discord.ui.Select):
        """選擇 API 密鑰類型"""
        try:
