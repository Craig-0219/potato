...
class LotteryStatsDashboardView(ui.View):
    ...
    @ui.select(
        ...
        options=[
            discord.SelectOption(label="最近 7 天", description="查看近一週的抽獎統計", emoji="📅", value="7"),
            discord.SelectOption(label="最近 30 天", description="查看近一個月的抽獎統計", emoji="📊", value="30"),
            discord.SelectOption(label="最近 90 天", description="查看近三個月的抽獎統計", emoji="📈", value="90"),
            discord.SelectOption(label="最近 365 天", description="查看近一年的抽獎統計", emoji="📋", value="365")
        ]
    )
    ...
    async def _create_stats_embed(self, stats: Dict[str, Any], period: int) -> discord.Embed:
        ...
        embed.add_field(
            ...
            value=f"**總抽獎數**: {total}\n"
                  f"**進行中**: {active} 🟢\n"
                  f"**已完成**: {completed} ✅\n"
                  f"**完成率**: {(completed/total*100):.1f}%" if total > 0 else "**完成率**: 0%",
            inline=True
        )
        ...
        if total > 0:
            ...
            activity_text = "🔥 非常活躍" if activity_score > 80 else \
                           "🟢 活躍" if activity_score > 60 else \
                           "🟡 一般" if activity_score > 30 else "🔴 較少"
