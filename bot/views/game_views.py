...
class RockPaperScissorsView(ui.View):
    ...
    async def _play_game(self, interaction: discord.Interaction, player_choice: str, player_emoji: str):
        """進行遊戲"""
        try:
            # 電腦隨機選擇
...
class CoinFlipView(ui.View):
            ...
            if embed:
                ...
                for i, field in enumerate(embed.fields):
                    if "下注金額" in field.name:
                        embed.set_field_at(i, name="💰 目前下注", 
                                          value=f"{self.bet_amount}🪙", inline=True)
                        break
                else:
    ...
    async def _play_coin_flip(self, interaction: discord.Interaction, choice: str, choice_emoji: str):
        """進行拋硬幣遊戲"""
        try:
...
class RouletteView(ui.View):
    ...
    async def _spin_roulette(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 更新經濟狀態
            self.user_economy['coins'] = self.user_economy.get('coins', 0) - self.bet_amount + winnings
    ...
    async def roll_dice(self, interaction: discord.Interaction, button: ui.Button):
        ...
        try:
            ...
            if self.prediction == "small" and 3 <= total <= 8:
                ...
                if (sorted_dice == [1, 2, 3] or sorted_dice == [2, 3, 4] or 
                    sorted_dice == [3, 4, 5] or sorted_dice == [4, 5, 6]):
                    won = True
                    multiplier = 5
