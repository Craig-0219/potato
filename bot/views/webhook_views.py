...
from bot.services.webhook_manager import webhook_manager, WebhookEvent, WebhookType, WebhookStatus
...
class WebhookManagerView(ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("❌ 只有原始命令使用者可以操作此介面", ephemeral=True)
            return False
        return True
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not self.url.value.startswith(('http://', 'https://')):
                await interaction.followup.send("❌ 請提供有效的URL (必須以http://或https://開頭)", ephemeral=True)
                return
        ...
        try:
            ...
            if self.headers.value and self.headers.value.strip() != "{}":
                try:
                    ...
                    await interaction.followup.send("❌ 自定義請求頭格式錯誤，請使用有效的JSON格式", ephemeral=True)
                    return
            ...
            if self.timeout.value:
                try:
                    ...
                    if 1 <= timeout_val <= 300:
                        ...
                        await interaction.followup.send("❌ 超時時間必須在1-300秒之間", ephemeral=True)
                        return
                except ValueError:
            ...
            if self.status.value:
                ...
                if status_val in ['active', 'inactive', 'paused', 'error']:
                    ...
                    await interaction.followup.send("❌ 狀態必須是 active, inactive, paused 或 error", ephemeral=True)
                    return
