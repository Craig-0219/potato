...
class MusicMenuView(discord.ui.View, SafeInteractionMixin):
    ...
    @discord.ui.button(label='🎛️ 控制面板', style=discord.ButtonStyle.secondary, custom_id='menu_control')
    async def control_panel_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """控制面板按鈕 - 重寫版"""
        try:
    ...
    @discord.ui.button(label='🔍 搜索音樂', style=discord.ButtonStyle.secondary, custom_id='menu_search')
    async def search_music_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """搜索音樂按鈕 - 重寫版"""
        try:
...
class MusicInputModal(discord.ui.Modal, title="🎵 播放音樂"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        """提交處理 - 重寫版"""
        try:
            logger.info(f"音樂輸入模態框提交 - 用戶: {interaction.user.name}, 輸入: {self.music_input.value}")
...
class SearchInputModal(discord.ui.Modal, title="🔍 搜索音樂"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        """提交處理 - 重寫版"""
        try:
            logger.info(f"搜索模態框提交 - 用戶: {interaction.user.name}, 搜索: {self.search_input.value}")
