        ...
        try:
            ...
            if top_keywords:
                embed.add_field(
                    name="🏷️ 熱門關鍵詞",
                    value=" • ".join(
                        f"**{keyword}**" for keyword in top_keywords[:8]
                    ),
                    inline=False,
                )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not result.success:
                await interaction.followup.send(
                    f"❌ 分析失敗：{result.error_message}", ephemeral=True
                )
                return
            ...
            if result.keywords:
                embed.add_field(
                    name="🏷️ 關鍵詞",
                    value=" • ".join(
                        f"**{kw}**" for kw in result.keywords[:8]
                    ),
                    inline=False,
                )
...
class SafetyCheckModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not result.success:
                await interaction.followup.send(
                    f"❌ 檢測失敗：{result.error_message}", ephemeral=True
                )
                return
            ...
            if toxicity:
                ...
                if toxicity.flagged_phrases:
                    embed.add_field(
                        ...
                        value=" • ".join(
                            f"**{phrase}**"
                            for phrase in toxicity.flagged_phrases[:5]
                        ),
                        inline=False,
                    )
                    ...
                    for category, score in toxicity.categories.items():
                        if score > 0.1:
                            ...
                            category_text.append(
                                f"{category_names.get(category, category)}: {score:.1%}"
                            )
        ...
        try:
            ...
            for i, link in enumerate(result.links[:3]):  # 最多顯示3個連結
                ...
                if link.risk_factors:
                    link_info += (
                        f"**風險因子**: {', '.join(link.risk_factors[:3])}"
                    )

                embed.add_field(
                    name=f"🔗 連結 {i + 1}", value=link_info, inline=False
                )
