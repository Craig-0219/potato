...
class MinecraftEventManager:
    ...
    async def create_event(self, organizer_id: int, guild_id: int, event_data: Dict[str, Any]) -> Optional[int]:
        """建立新活動"""
        try:
            # 驗證必要欄位
    ...
    async def register_participant(self, event_id: int, discord_id: int, minecraft_uuid: str = None) -> bool:
        """玩家報名參與活動"""
        try:
            # 檢查活動是否存在且開放報名
