...
class PlayerManager:
    ...
    async def bind_player(
        self, discord_id: int, minecraft_uuid: str, minecraft_username: str
    ) -> bool:
        ...
        try:
            ...
            if existing:
                logger.warning(
                    f"Discord 用戶 {discord_id} 已綁定到 {existing['minecraft_username']}"
                )
                return False
            ...
            if existing_discord:
                logger.warning(
                    f"Minecraft 玩家 {minecraft_username} 已被 Discord 用戶 {existing_discord['discord_id']} 綁定"
                )
                return False
            ...
            logger.info(
                f"成功綁定 Discord {discord_id} ↔ Minecraft {minecraft_username} ({minecraft_uuid})"
            )
            return True
    ...
    async def get_top_players(
        self, stat_type: str = "total_playtime", limit: int = 10
    ) -> List[Dict[str, Any]]:
        """獲取排行榜玩家"""
        try:
            valid_stats = [
