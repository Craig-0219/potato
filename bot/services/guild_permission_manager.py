...
class GuildPermissionManager:
    ...
    async def check_permission(self, user_id: int, guild_id: int, 
                              permission: GuildPermission,
                              check_discord_perms: bool = True) -> bool:
        """檢查用戶是否擁有特定權限"""
        try:
            # 獲取用戶權限
    ...
    async def get_user_permissions(self, user_id: int, guild_id: int) -> GuildUserPermissions:
        ...
        if cache_key in self._permission_cache:
            ...
            # 檢查快取是否過期（5分鐘）
            if cached.assigned_at and (
                datetime.now() - cached.assigned_at
            ).total_seconds() < 300:
                return cached
    ...
    async def assign_role(self, user_id: int, guild_id: int, 
                         role: GuildRole, assigned_by: int,
    ...
    async def _assign_role_to_user(self, user_id: int, guild_id: int, 
                                  role: GuildRole, assigned_by: int,
                                  expires_at: Optional[datetime] = None) -> bool:
        """內部角色分配方法"""
        try:
            ...
            async with self.db.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (
                        user_id, guild_id, permission_data["roles"],
                        assigned_by, permission_data["assigned_at"],
                        expires_at, True
                    ))
                    await conn.commit()
            ...
            logger.info(f"✅ 角色分配成功: 用戶 {user_id} 在伺服器 {guild_id} 獲得 {role.value} 角色")
            return True
    ...
    async def remove_role(self, user_id: int, guild_id: int, 
                         role: GuildRole, removed_by: int) -> bool:
        """移除用戶角色"""
        try:
            # 檢查權限
    ...
    async def _check_discord_permission(self, user_id: int, guild_id: int, 
                                       permission: GuildPermission) -> bool:
        """檢查 Discord 原生權限"""
        try:
