...
class AIManager:
    ...
    async def suggest_reply(self, ticket_content: str, ticket_context: Dict[str, Any]) -> Dict[str, Any]:
        """分析票券內容並建議回覆"""
        try:
            # 1. 內容分析
    ...
    async def _generate_reply_suggestions(self, analysis: Dict[str, Any], context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """生成回覆建議"""
        suggestions = []
        ...
        # 根據類型選擇模板
        templates = self.response_templates.get(ticket_type, self.response_templates.get('general', []))
    ...
    async def suggest_tags(self, ticket_content: str, ticket_type: str = None) -> List[Dict[str, Any]]:
        """建議適合的標籤"""
        try:
            suggestions = []
            ...
            for suggestion in suggestions:
                ...
                if tag_name not in unique_suggestions or suggestion['confidence'] > unique_suggestions[tag_name]['confidence']:
    ...
    async def assess_priority(self, ticket_content: str, user_context: Dict[str, Any]) -> Dict[str, Any]:
        """智能評估票券優先級"""
        try:
            analysis = await self._analyze_content(ticket_content)
    ...
    def _extract_keywords(self, content: str) -> List[str]:
        ...
        # 技術相關關鍵字
        tech_keywords = ['api', 'database', 'server', 'client', 'bug', 'error', '錯誤', '伺服器', '資料庫']
        for keyword in tech_keywords:
            if keyword in content_lower:
                keywords.append(keyword)
        
        # 功能相關關鍵字
        feature_keywords = ['login', '登入', 'register', '註冊', 'payment', '付款', 'profile', '個人資料']
    ...
    def _load_keyword_rules(self) -> Dict[str, Any]:
        """載入關鍵字分析規則"""
        return {
            'urgency_keywords': ['急', '緊急', '馬上', '立即', '火急', 'urgent', 'asap', 'emergency'],
            'positive_keywords': ['謝謝', '感謝', '很好', '棒', '滿意', 'thanks', 'good', 'great'],
            'negative_keywords': ['生氣', '憤怒', '爛', '糟糕', '討厭', 'angry', 'bad', 'terrible'],
            'technical_keywords': ['api', 'bug', 'error', 'server', 'database', '錯誤', '伺服器', '資料庫'],
