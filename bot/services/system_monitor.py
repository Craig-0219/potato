...
class SystemMonitor:
    ...
    async def _collect_bot_metrics(self):
        ...
        try:
            ...
            await self._record_metric('bot', 'latency', latency_ms, 'ms', 'performance')
            
            # 伺服器數量
            guild_count = len(self.bot.guilds)
            await self._record_metric('bot', 'guild_count', guild_count, 'count', 'bot')
            
            # 用戶數量
            user_count = len(self.bot.users)
            await self._record_metric('bot', 'user_count', user_count, 'count', 'bot')
            ...
            if hasattr(self.bot, '_connection'):
                ...
                if hasattr(connection, '_messages'):
                    ...
                    await self._record_metric('bot', 'cached_messages', cached_messages, 'count', 'cache')
            ...
            for guild in self.bot.guilds:
                ...
                await self._record_metric(guild_id, 'online_members', online_members, 'count', 'guild')
        ...
        try:
            async with self.db.connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    # 活躍票券數
                    await cursor.execute("SELECT COUNT(*) as count FROM tickets WHERE status = 'open'")
                    result = await cursor.fetchone()
                    active_tickets = result['count'] if result else 0
                    await self._record_metric('tickets', 'active_tickets', active_tickets, 'count', 'tickets')
                    ...
                    await self._record_metric('tickets', 'today_new_tickets', today_tickets, 'count', 'tickets')
                    ...
                    await self._record_metric('tickets', 'today_closed_tickets', today_closed, 'count', 'tickets')
                    ...
                    """)
                    result = await cursor.fetchone()
                    avg_response = result['avg_response'] if result and result['avg_response'] else 0
                    await self._record_metric('tickets', 'avg_response_time', float(avg_response), 'minutes', 'performance')
                    ...
                    """)
                    result = await cursor.fetchone()
                    avg_resolution = result['avg_resolution'] if result and result['avg_resolution'] else 0
                    await self._record_metric('tickets', 'avg_resolution_time', float(avg_resolution), 'hours', 'performance')
                    ...
                    if result and result['avg_rating']:
                        await self._record_metric('tickets', 'avg_rating', float(result['avg_rating']), 'stars', 'satisfaction')
                        await self._record_metric('tickets', 'rating_count_7d', result['rating_count'], 'count', 'satisfaction')
                    
        except Exception as e:
            logger.error(f"收集票券指標失敗: {e}")
    
    async def _record_metric(self, guild_id: str, metric_name: str, value: float, 
    ...
    async def get_metric_history(self, guild_id: str, metric_name: str, 
    ...
    async def get_system_health_score(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_response = ticket_metrics.get('avg_response_time', {}).get('value', 0)
    ...
    async def generate_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            for guild_id, metric_name in key_metrics:
                history = await self.get_metric_history(guild_id, metric_name, hours)
                if history:
