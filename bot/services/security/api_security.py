...
from bot.services.security.audit_manager import audit_manager, SecurityEvent, EventCategory, EventSeverity
...
class APISecurityManager:
    ...
    async def create_api_key(self, name: str, key_type: APIKeyType, 
                           user_id: Optional[int] = None, guild_id: Optional[int] = None,
                           permissions: List[str] = None, 
                           expires_at: Optional[datetime] = None) -> Optional[APIKey]:
        """
        創建 API 密鑰
        ...
        try:
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (
                        key_value, name, key_type.value, user_id, guild_id,
                        ','.join(permissions or []), True, datetime.now(), expires_at
                    ))
                    ...
                    for rule in rate_limits:
                        ...
                        """, (
                            api_key_id, rule.limit_type.value, rule.max_requests,
                            rule.window_seconds, rule.burst_allowance
                        ))
            ...
            # 記錄安全事件
            await audit_manager.log_event(SecurityEvent(
                user_id=user_id,
                guild_id=guild_id,
    ...
    async def check_rate_limit(self, api_key: str, client_ip: str = None) -> Tuple[bool, Dict[str, Any]]:
        """
        檢查速率限制
            ...
            for rule in api_key_obj.rate_limits:
                ...
                if current_requests >= rule.max_requests:
                    ...
                    return False, {
                        "error": "rate_limit_exceeded",
                        "rule": rule.limit_type.value,
                        "retry_after": window_size - (current_time - min(request_times)) if request_times else 0,
    ...
    async def generate_jwt_token(self, user_id: int, permissions: List[str] = None,
        ...
        try:
            ...
            await audit_manager.log_event(SecurityEvent(
                ...
                severity=EventSeverity.INFO,
                message="JWT 令牌已生成",
                details={"expires_in": expires_in, "permissions": permissions or []}
            ))
