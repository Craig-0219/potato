...
class RBACManager:
        ...
        try:
            default_roles = [
                ...
                },
                {
                    ...
                    "permissions": [
                        Permission.SYSTEM_VIEW, Permission.USER_MANAGE, Permission.USER_VIEW,
                        Permission.USER_BAN, Permission.USER_MUTE, Permission.TICKET_ADMIN,
                        Permission.VOTE_ADMIN, Permission.ECONOMY_ADMIN, Permission.AI_CONFIG,
                        Permission.CONTENT_ADMIN, Permission.WORKFLOW_ADMIN, Permission.API_READ,
                ...
                },
                {
                    ...
                    "permissions": [
                        Permission.USER_VIEW, Permission.USER_MUTE, Permission.TICKET_MANAGE,
                        Permission.VOTE_MANAGE, Permission.CONTENT_MODERATE, Permission.AI_USE,
                        Permission.API_READ
                    ],
                    "is_system": True
                },
                {
                    ...
                    "permissions": [
                        Permission.USER_VIEW, Permission.TICKET_VIEW, Permission.TICKET_CLOSE,
                        Permission.VOTE_CREATE, Permission.VOTE_PARTICIPATE, Permission.ECONOMY_VIEW,
                        Permission.AI_USE, Permission.CONTENT_ANALYZE, Permission.WORKFLOW_EXECUTE,
                        Permission.API_READ
                    ],
                    "is_system": True
                },
                {
                    ...
                    "level": RoleLevel.VIP,
                    "permissions": [
                        Permission.VOTE_CREATE, Permission.VOTE_PARTICIPATE, 
                        Permission.ECONOMY_TRANSACTION, Permission.AI_USE,
                        Permission.CONTENT_ANALYZE, Permission.WORKFLOW_EXECUTE
                    ],
                    "is_system": True
                },
                {
                    ...
                    "permissions": [
                        Permission.VOTE_PARTICIPATE, Permission.ECONOMY_TRANSACTION, 
                        Permission.AI_USE
                    ],
                    "is_system": True
                }
    ...
    async def create_role(self, name: str, description: str, level: RoleLevel, 
                         permissions: List[Permission] = None, is_system: bool = False) -> Optional[Role]:
        """
        創建角色
    ...
    async def assign_role(self, user_id: int, guild_id: int, role_id: int, 
                         assigned_by: int, expires_at: Optional[datetime] = None) -> bool:
        """
        為用戶分配角色
        ...
        try:
            ...
            # 記錄操作日誌
            await self._log_rbac_event(user_id, guild_id, "role_assigned", {
                "role_id": role_id,
    ...
    async def revoke_role(self, user_id: int, guild_id: int, role_id: int, revoked_by: int) -> bool:
        ...
        try:
            ...
            # 記錄操作日誌
            await self._log_rbac_event(user_id, guild_id, "role_revoked", {
                "role_id": role_id,
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    if include_inactive:
                        ...
                        await cursor.execute("SELECT id FROM rbac_roles WHERE is_active = TRUE ORDER BY level DESC, name")
    ...
    def _is_cache_valid(self, cache_key: str) -> bool:
        ...
        return (datetime.now() - self._last_cache_update[cache_key]).total_seconds() < self.cache_ttl
    
    async def _log_rbac_event(self, user_id: int, guild_id: int, event_type: str, details: Dict[str, Any]):
        """記錄 RBAC 事件"""
        try:
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (user_id, guild_id, f"rbac_{event_type}", json.dumps(details), datetime.now()))
                    await conn.commit()
        except Exception as e:
