...
class MFAManager:
                ...
                async with conn.cursor() as cursor:
                    ...
                    for code in backup_codes:
                        ...
                        await cursor.execute("""
                            INSERT INTO user_mfa (user_id, method_type, secret_key, is_enabled, created_at)
                            VALUES (%s, %s, %s, %s, %s)
                        """, (user_id, MFAMethod.BACKUP_CODES.value, encrypted_code, True, datetime.now()))
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if result:
                        ...
                        # 記錄成功使用
                        await self._log_mfa_event(user_id, MFAMethod.BACKUP_CODES, True, "備用代碼驗證成功")
                        ...
                        return {
                            ...
                            "warning": f"剩餘 {remaining_codes} 個備用代碼" if remaining_codes > 0 else "備用代碼已用完，請重新生成"
                        }
                    else:
                        # 代碼無效
                        await self._increment_attempt_count(user_id, MFAMethod.BACKUP_CODES)
                        await self._log_mfa_event(user_id, MFAMethod.BACKUP_CODES, False, "備用代碼無效")
    ...
    async def disable_mfa_method(self, user_id: int, method: MFAMethod, verification_code: str = None) -> Dict[str, Any]:
        """
        停用 MFA 方法
        ...
        try:
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (
                        user_id, "mfa_verification", method.value, success, details,
                        "internal", "discord_bot", datetime.now()
                    ))
                    await conn.commit()
        except Exception as e:
    ...
    async def _get_security_recommendations(self, user_id: int, methods: Dict) -> List[str]:
        ...
        if MFAMethod.TOTP.value not in methods or not methods.get(MFAMethod.TOTP.value, {}).get("enabled"):
