...
class AuditManager:
        ...
        try:
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """, (
                        ...
                        event.timestamp, event.hash_signature
                    ))
            ...
            if event.user_id:
                ...
                if len(self._user_activity_cache[event.user_id]) > 100:
                    self._user_activity_cache[event.user_id] = self._user_activity_cache[event.user_id][-50:]
    ...
    async def log_authentication_event(self, user_id: int, event_type: str, 
    ...
    async def log_authorization_event(self, user_id: int, guild_id: int, 
    ...
    async def log_data_access_event(self, user_id: int, action: str, 
    ...
    async def log_api_access_event(self, user_id: Optional[int], endpoint: str,
    ...
    async def detect_suspicious_activity(self, user_id: int = None, 
        ...
        try:
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    """
                    params = [
                        EventCategory.AUTHENTICATION.value, '%login%', start_time,
                        EventSeverity.MEDIUM.value, EventSeverity.HIGH.value
                    ]
                    ...
                    """, (EventCategory.API_ACCESS.value, start_time, self.max_events_per_minute * 60))
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if standard == ComplianceStandard.SOC2:
                        report.update(await self._generate_soc2_compliance_data(cursor, start_date, end_date, guild_id))
                    elif standard == ComplianceStandard.GDPR:
                        report.update(await self._generate_gdpr_compliance_data(cursor, start_date, end_date, guild_id))
    ...
    async def verify_log_integrity(self, start_date: datetime = None, 
        ...
        try:
            ...
            result = {
                ...
                "status": "good" if integrity_percentage > 99 else "warning" if integrity_percentage > 95 else "critical"
            }
    ...
    def _calculate_record_hash(self, event_id: int, user_id: Optional[int], 
                             event_type: str, timestamp: datetime, details: str) -> str:
        """計算記錄雜湊值"""
    ...
    async def _detect_threats(self, event: SecurityEvent) -> bool:
        ...
        if event.ip_address:
            try:
                ...
                # 檢測私有 IP 範圍外的可疑存取
                if not ip.is_private and self._ip_activity_cache[event.ip_address] > self.suspicious_ip_threshold:
                    return True
            except ValueError:
                pass
    ...
    async def _detect_anomalies(self, event: SecurityEvent):
        ...
        if event.user_id:
            # 檢測用戶活動頻率異常
            recent_events = [
                ts for ts in self._user_activity_cache[event.user_id]
    ...
    async def _generate_soc2_compliance_data(self, cursor, start_date, end_date, guild_id) -> Dict[str, Any]:
        """生成 SOC 2 合規數據"""
        # SOC 2 相關的合規檢查
        return {
            "soc2_controls": {
    ...
    async def _generate_gdpr_compliance_data(self, cursor, start_date, end_date, guild_id) -> Dict[str, Any]:
        """生成 GDPR 合規數據"""
        # GDPR 相關的合規檢查
        return {
            "gdpr_controls": {
    ...
    async def _calculate_compliance_score(self, event_stats: Dict, standard: ComplianceStandard) -> int:
        """計算合規分數"""
        # 簡化的合規分數計算
        base_score = 100
