...
class GameManager:
    ...
    async def create_game_session(self, game_type: str, player_id: int, 
                                guild_id: int, **kwargs) -> str:
        """創建遊戲會話"""
        try:
    ...
    async def get_leaderboard(self, guild_id: int, game_type: str, 
                            metric: str = "wins", limit: int = 10) -> List[Dict[str, Any]]:
        """獲取排行榜"""
        try:
            # 快取鍵
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    for i, row in enumerate(results, 1):
                        (player_id, total_games, total_wins, avg_score, 
                         best_score, win_rate) = row
    ...
    def spin_roulette(self, bet_amount: int, bet_type: str, bet_value: Any = None) -> Dict[str, Any]:
        """輪盤遊戲"""
        # 生成結果數字 (0-36)
