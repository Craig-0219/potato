...
class DataExportManager:
    ...
    async def export_data(self, request: ExportRequest) -> ExportResult:
        ...
        try:
            ...
            # 匯出資料
            actual_file_path = await self._write_export_file(data, file_path, request.format, metadata)
    ...
    async def _export_security_events(self, request: ExportRequest) -> tuple[List[Dict], Dict]:
        ...
        metadata = {
            ...
            'filtered_sensitive': not (request.filters and request.filters.get('include_sensitive') == True),
            'export_timestamp': datetime.now().isoformat()
        }
        
        return events, metadata
    
    async def _write_export_file(self, data: List[Dict], file_path: Path, format: str, metadata: Dict):
        """寫入匯出檔案"""
        if format == 'csv':
        ...
        def write_excel_sync():
            ...
            with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                ...
                metadata_df = pd.DataFrame([{
                    ...
                    '篩選條件': json.dumps(metadata.get('filters_applied', {}), ensure_ascii=False)
                }])
    ...
    async def _log_export(self, request: ExportRequest, file_path: str, record_count: int, export_time: float):
        """記錄匯出日誌"""
        try:
            async with self.db.connection() as conn:
    ...
    async def create_bulk_export(self, export_types: List[str], format: str = 'json', 
                               guild_id: Optional[int] = None, 
                               date_range: Optional[tuple] = None) -> ExportResult:
        """創建批量匯出（ZIP 壓縮檔）"""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                zipf.writestr("export_summary.json", 
                            json.dumps(summary, ensure_ascii=False, indent=2, default=str))
            
            file_size = zip_path.stat().st_size
            
            logger.info(f"✅ 批量匯出完成: {zip_filename} ({successful_exports}/{len(export_types)} 成功)")
    ...
    async def get_export_history(self, days: int = 30, guild_id: Optional[int] = None) -> List[Dict]:
        """獲取匯出歷史記錄"""
        try:
