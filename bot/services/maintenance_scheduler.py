...
class MaintenanceScheduler:
    ...
    def _setup_schedules(self):
        ...
        for task in self.tasks.values():
            ...
            if task.frequency == TaskFrequency.HOURLY:
                ...
                getattr(schedule.every(), run_day).at(run_time).do(
    ...
    async def _execute_task(self, task_id: str):
        ...
        try:
            ...
            result = await asyncio.wait_for(
                task.handler(task.config or {}), timeout=task.timeout_seconds
            )
    ...
    async def _daily_cleanup_task(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "details": {
                    k: {
                        "deleted": getattr(v, "deleted_count", 0),
                        "success": getattr(v, "success", False),
                    }
    ...
    async def get_execution_history(
        self, task_id: Optional[str] = None, days: int = 7
    ) -> List[Dict[str, Any]]:
        """獲取執行歷史記錄"""
        try:
