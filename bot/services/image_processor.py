...
class ImageProcessor:
    ...
    def __init__(self):
        ...
        # 壓縮品質設定
        self.quality_presets = {"high": 90, "medium": 75, "low": 60}
    ...
    async def process_image(
        ...
        try:
            ...
            output_data = await self._convert_format(
                processed_image, request.output_format, request.quality
            )
    ...
    async def _apply_operation(
        self, image: Image.Image, request: ImageProcessRequest
    ...
    async def _convert_format(
        self, image: Image.Image, output_format: ImageFormat, quality: int
    ) -> bytes:
        """轉換圖片格式"""
        try:
            output = io.BytesIO()
