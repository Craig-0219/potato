...
class AchievementManager:
    ...
    async def check_daily_achievements(self, user_id: int, guild_id: int, 
    ...
    async def check_game_achievements(self, user_id: int, guild_id: int, 
                                    game_type: str, won: bool, score: int) -> List[Dict[str, Any]]:
        """檢查遊戲相關成就"""
        achievements_earned = []
        ...
        try:
            ...
            if 2 <= current_hour <= 4:
                if not await self._user_has_achievement(user_id, guild_id, "late_night_gamer"):
                    achievement = await self._grant_achievement(user_id, guild_id, "late_night_gamer")
                    if achievement:
                        achievements_earned.append(achievement)
        ...
        try:
            ...
            for achievement_id, required_coins in coin_achievements:
                if coins >= required_coins:
                    if not await self._user_has_achievement(user_id, guild_id, achievement_id):
                        achievement = await self._grant_achievement(user_id, guild_id, achievement_id)
                        if achievement:
                            achievements_earned.append(achievement)
    ...
    async def _grant_achievement(self, user_id: int, guild_id: int, achievement_id: str) -> Optional[Dict[str, Any]]:
        """授予成就"""
        try:
    ...
    async def _grant_achievement_rewards(self, user_id: int, guild_id: int, achievement: Achievement):
        """發放成就獎勵"""
        try:
            from bot.services.economy_manager import EconomyManager
            economy_manager = EconomyManager()
            ...
            if achievement.rewards.get("experience", 0) > 0:
                await economy_manager.add_experience(user_id, guild_id, achievement.rewards["experience"])
    ...
    async def get_achievement_progress(self, user_id: int, guild_id: int, achievement_id: str) -> Dict[str, Any]:
        """獲取成就進度"""
        try:
            achievement_def = self.achievements.get(achievement_id)
            if not achievement_def:
            ...
            # 獲取當前進度（根據成就類型計算）
            progress = await self._calculate_achievement_progress(user_id, guild_id, achievement_def)
    ...
    async def _calculate_achievement_progress(self, user_id: int, guild_id: int, 
                                           achievement: Achievement) -> Dict[str, Any]:
        """計算成就進度"""
        try:
            from bot.services.economy_manager import EconomyManager
            economy_manager = EconomyManager()
