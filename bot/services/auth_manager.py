...
class AuthenticationManager:
    ...
    async def sync_discord_user(self, discord_user: discord.Member, 
                               password: str = None) -> Tuple[bool, str, Optional[AuthUser]]:
        ...
        try:
            ...
            if is_admin:
                ...
                permissions = ['tickets.read', 'tickets.write', 'tickets.assign', 
                             'tickets.close', 'statistics.read', 'users.read']
            else:
            ...
            async with self.db.connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    logger.info(f"Discord 用戶同步成功: {discord_user.display_name}")
    ...
    async def authenticate_user(self, discord_id: str, password: str, 
                               guild_id: int) -> Tuple[bool, str, Optional[AuthUser]]:
        """用戶密碼認證"""
        try:
            async with self.db.connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
    ...
    async def create_api_key(self, user_id: int, name: str, 
                           ...
                           expires_days: int = 30) -> Tuple[bool, str, Optional[str]]:
        """創建 API 金鑰"""
        try:
            # 生成金鑰
    ...
    async def create_login_session(self, user_id: int, ip_address: str = None, 
    ...
    def check_permission(self, auth_user: AuthUser, required_permission: str) -> bool:
        """檢查用戶權限"""
