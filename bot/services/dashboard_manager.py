...
class DashboardManager:
    ...
    async def generate_overview_dashboard(
        ...
        try:
            ...
            # 2. 工作流程效率圖 (柱狀圖)
            charts.append(
                await self._create_workflow_efficiency_chart(workflow_data)
            )
            ...
            # 4. 系統性能熱力圖
            charts.append(
                await self._create_performance_heatmap(performance_data)
            )
    ...
    async def generate_performance_dashboard(
        ...
        try:
            # 獲取性能數據
            performance_data = await self._get_detailed_performance_data(
                guild_id, days
            )
            ...
            # 1. 回應時間趨勢 (折線圖)
            charts.append(
                await self._create_response_time_chart(performance_data)
            )

            # 2. 系統負載分佈 (柱狀圖)
            charts.append(
                await self._create_load_distribution_chart(performance_data)
            )

            # 3. SLA合規性圓餅圖
            charts.append(
                await self._create_sla_compliance_chart(performance_data)
            )

            # 4. 客服工作量熱力圖
            charts.append(
                await self._create_workload_heatmap(performance_data)
            )
    ...
    async def generate_predictive_dashboard(
        ...
        try:
            ...
            # 1. 票券量預測 (面積圖)
            charts.append(
                await self._create_volume_prediction_chart(historical_data)
            )

            # 2. 工作負載預測 (折線圖)
            charts.append(
                await self._create_workload_prediction_chart(historical_data)
            )

            # 3. 資源需求預測 (柱狀圖)
            charts.append(
                await self._create_resource_prediction_chart(historical_data)
            )

            # 4. 趨勢分析散點圖
            charts.append(
                await self._create_trend_analysis_chart(historical_data)
            )
    ...
    async def _get_ticket_analytics(
    ...
    async def _get_workflow_analytics(
        self, guild_id: int, days: int
    ) -> Dict[str, Any]:
        """獲取工作流程分析數據"""
        try:
            # 獲取工作流程統計
            workflow_stats = (
                await self.workflow_dao.get_guild_workflow_statistics(
                    guild_id, days
                )
            )
            ...
            guild_executions = [
                ...
                and self.workflow_dao.get_workflow(e["workflow_id"])[
                    "guild_id"
                ]
                == guild_id
            ]
    ...
    async def _create_workload_heatmap(
        ...
        for day_idx in range(7):
            for hour_idx in range(12):
                ...
                workload = min(
                    100, base_workload * time_factor + (day_idx * 5)
                )
        ...
        for day_idx in range(7):
            ...
            for hour in range(24):
                ...
                if 9 <= hour <= 17:  # 工作時間
                    ...
                    score = (
                        base_score - 20 + (10 * (1 - min(hour, 24 - hour) / 6))
                    )
            ...
            datasets=[
                {
                    "label": "性能分數",
                    "data": heatmap_data,
                    "backgroundColor": lambda value: self._get_heatmap_color(
                        value
                    ),
                }
            ],
    ...
    async def _create_volume_prediction_chart(
        ...
        prediction_dates = [
            (datetime.now(timezone.utc) + timedelta(days=i + 1)).strftime(
                "%m-%d"
            )
        ...
        historical_data_extended = historical_volumes + [None] * len(
            predicted_volumes
        )
    ...
    async def _create_workload_prediction_chart(
        ...
        # 預測工作負載趨勢
        predicted_workload = self._predict_linear_trend(
            historical_workload, self._prediction_window
        )
    ...
    async def _create_trend_analysis_chart(
        ...
        for i in range(len(ticket_counts)):
            scatter_data.append(
                {"x": ticket_counts[i], "y": resolution_times[i]}
            )
    ...
    def _predict_linear_trend(
        ...
        if len(data) < 2:
            return (
                [data[-1]] * prediction_days if data else [0] * prediction_days
            )
    ...
    async def _generate_key_metrics(
        ...
        try:
            # 獲取當前期間和前一期間的數據進行比較
            current_stats = (
                await self.stats_manager.generate_comprehensive_report(
                    guild_id, days
                )
            )
            previous_stats = (
            ...
            metrics["response_time"] = MetricSummary(
                ...
                trend=self._determine_trend(
                    previous_response_time, current_response_time
                ),  # 反向，越低越好
    ...
    async def _generate_insights(
        ...
        try:
            ...
            if daily_data:
                recent_avg = (
                    sum(day.get("created_count", 0) for day in daily_data[-7:])
                    / 7
                )
                overall_avg = sum(
    ...
    def _determine_status(
        ...
        """確定指標狀態"""
        change_rate = (
            abs((current - previous) / previous) if previous > 0 else 0
        )
    ...
    async def _calculate_engagement_score(
        self, welcome_stats: Dict[str, Any], vote_stats: Dict[str, Any]
    ) -> float:
        """計算用戶參與度分數"""
        try:
            # 基於歡迎和投票數據計算參與度
            ...
            vote_score = min(
                vote_stats.get("total_votes", 0) / 5, 30
            )  # 最多30分
    ...
    async def _calculate_trends(
        ...
        try:
            ...
            # 取最近7天和前7天的數據進行比較
            recent_values = [
                day.get("created_count", 0) for day in daily_data[-7:]
            ]
            previous_values = (
                [day.get("created_count", 0) for day in daily_data[-14:-7]]
    ...
    async def _calculate_workflow_efficiency(
        ...
        try:
            ...
            failed_executions = [
                e for e in executions if e.get("status") == "failed"
            ]
    ...
    async def _get_detailed_performance_data(
        ...
        try:
            # 獲取票券性能指標
            ticket_metrics = (
                await self.ticket_dao.get_ticket_performance_metrics(
                    guild_id, days
                )
            )
    ...
    async def _get_historical_data_for_prediction(
        ...
        try:
            ...
            daily_stats = await self.ticket_dao.get_daily_ticket_stats(
                guild_id, start_date, end_date
            )

            # 提取票券量數據
            daily_volumes = [
                stats.get("created_count", 0) for stats in daily_stats
            ]
            ...
            return {
                ...
                "data_quality": (
                    "good" if len(daily_volumes) >= 30 else "limited"
                ),
            }
    ...
    async def _generate_predictive_metrics(
        ...
        try:
            ...
            data_consistency = (
                1.0 / (1.0 + data_variance) if data_variance > 0 else 1.0
            )
            data_completeness = len(daily_volumes) / 60.0  # 基於60天完整度
            prediction_confidence = min(
                data_consistency * data_completeness, 1.0
            )
            ...
            return {
                "prediction_confidence": MetricSummary(
                    ...
                    change_percentage=round(
                        (prediction_confidence - 0.5) * 200, 1
                    ),
    ...
    async def _generate_predictive_insights(
        ...
        try:
            ...
            if recent_avg > overall_avg * 1.15:
                ...
                insights.append(
                    "📉 強烈下降趨勢：票券量顯著減少，可考慮調整服務策略"
                )
            elif recent_avg < overall_avg * 0.95:
                insights.append(
                    "📊 輕微下降趨勢：票券量略有下降，可能是積極的改善信號"
                )
            else:
            ...
            if predicted_volume and len(predicted_volume) > 0:
                ...
                if next_week_avg > overall_avg * 1.2:
                    insights.append(
                        "🚨 預警：預測下週票券量可能大幅增加，建議提前準備"
                    )
                elif next_week_avg > overall_avg * 1.1:
                    insights.append(
                        "⚡ 準備：預測票券量將略有增加，建議適度調整資源"
                    )
            ...
            if data_quality == "limited":
                insights.append(
                    "📋 數據提醒：歷史數據有限，預測準確性可能受到影響"
                )
            elif data_quality == "poor":
                insights.append(
                    "⚠️ 數據警告：數據品質不佳，建議改善數據收集機制"
                )
    ...
    async def get_dashboard_cache_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_usage": sum(
                len(str(dashboard))
                for dashboard in self._dashboard_cache.values()
            ),
        }
    ...
    async def _generate_performance_metrics(
        ...
        try:
            ...
            raw_metrics = {
                ...
                "response_latency": system_metrics.get(
                    "avg_response_ms", 150.0
                ),
                "error_rate": system_metrics.get("error_rate", 0.1),
                # 工作流程效率指標
                "workflow_success_rate": workflow_metrics.get(
                    "success_rate", 95.0
                ),
                "avg_workflow_time": workflow_metrics.get(
                    "avg_execution_time", 30.0
                ),
                "automation_coverage": workflow_metrics.get(
            ...
            previous_metrics = {
                "avg_resolution_time": raw_metrics["avg_resolution_time"]
                * 1.1,
                "resolution_rate": raw_metrics["resolution_rate"] * 0.95,
                "customer_satisfaction": raw_metrics["customer_satisfaction"]
                * 0.98,
                "first_response_time": raw_metrics["first_response_time"]
                * 1.2,
                ...
                "workflow_success_rate": raw_metrics["workflow_success_rate"]
                * 0.96,
                "avg_workflow_time": raw_metrics["avg_workflow_time"] * 1.05,
                "automation_coverage": raw_metrics["automation_coverage"]
                * 0.95,
            }
            ...
            score_components = [
                min(
                    100,
                    max(
                        0, 100 - (raw_metrics["avg_resolution_time"] - 2) * 10
                    ),
                ),
