...
class DatabaseCleanupManager:
    ...
    async def perform_comprehensive_cleanup(self, guild_id: int, cleanup_config: Dict[str, Any] = None) -> Dict[str, Any]:
        """執行全面的資料庫清理"""
        ...
        try:
            ...
            # 1. 歸檔舊票券
            logger.info(f"歸檔 {config['ticket_retention_days']} 天前的票券...")
            ticket_result = await self.dao.archive_old_tickets(
                guild_id, 
                config['ticket_retention_days'], 
                config['batch_size']
            )
            ...
            activity_result = await self.dao.archive_user_activity(guild_id, "monthly")
            ...
            logger.info(f"伺服器 {guild_id} 資料庫清理完成 - 歸檔: {total_archived}, 刪除: {total_cleaned}")
    ...
    async def archive_specific_period(self, guild_id: int, data_type: str, 
    ...
    async def _archive_tickets_by_period(self, guild_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """歸檔特定期間的票券"""
        # 這裡可以實現更精確的期間歸檔邏輯
        # 目前使用現有的方法，但可以擴展為更靈活的查詢
        return await self.dao.archive_old_tickets(guild_id, 0, 1000)  # 暫時實現

    async def _archive_votes_by_period(self, guild_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """歸檔特定期間的投票"""
        # 這裡可以實現更精確的期間歸檔邏輯
        return await self.dao.archive_old_votes(guild_id, 0, 1000)  # 暫時實現

    async def setup_cleanup_schedule(self, guild_id: int, schedule_config: Dict[str, Any]) -> Dict[str, Any]:
        """設置清理排程"""
        try:
    ...
    async def _optimize_database_indexes(self, guild_id: int) -> Dict[str, Any]:
        """優化資料庫索引"""
        try:
            # 這裡可以分析並優化資料庫索引
    ...
    async def _generate_cleanup_recommendations(self, guild_id: int) -> List[str]:
        """生成清理建議"""
        try:
            recommendations = []
            ...
            for guild in self.bot.guilds:
                try:
                    ...
                    for schedule in schedules:
                        if (schedule.get('next_run') and 
                            datetime.now() >= schedule['next_run'] and
                            schedule.get('is_enabled', True)):
                            
                            await self._execute_scheduled_cleanup(guild.id, schedule)
    ...
    async def _execute_scheduled_cleanup(self, guild_id: int, schedule: Dict):
        """執行排程清理"""
        try:
            logger.info(f"執行排程清理: {schedule['id']} - {schedule['cleanup_type']}")
            ...
            cleanup_config = {
                ...
                'archive_before_delete': schedule.get('archive_before_delete', True)
            }
