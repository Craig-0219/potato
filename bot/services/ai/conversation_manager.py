...
from .ai_engine_manager import AIEngineManager, ConversationContext, AIResponse
...
class ConversationManager:
    ...
    def __init__(self, ai_engine: AIEngineManager, intent_recognizer: IntentRecognizer, config: Dict[str, Any]):
    ...
    async def _validate_input(self, session: ConversationSession, step: ConversationStep, message: str) -> bool:
        """驗證用戶輸入"""
    ...
    async def _complete_ticket_creation(self, session: ConversationSession, confirmation: str) -> str:
        """完成票券創建"""
        if confirmation.lower().strip() in ["是", "yes", "y", "確認"]:
            ...
            # 轉換問題類型
            type_map = {"1": "bug_report", "2": "question", "3": "feature_request", "4": "technical_support"}
    ...
    async def list_active_sessions(self, user_id: Optional[str] = None) -> List[ConversationSession]:
        """列出活躍會話"""
