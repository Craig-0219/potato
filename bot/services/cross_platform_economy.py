...
class CrossPlatformEconomyManager:
    ...
    async def link_accounts(self, discord_id: int, minecraft_uuid: str, 
            ...
            async with db_pool.connection() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute("""
                        ...
                        VALUES (%s, %s, %s, %s)
                    """, (discord_id, minecraft_uuid, guild_id, datetime.now(timezone.utc)))
                    await conn.commit()
    ...
    async def unlink_accounts(self, discord_id: int = None, 
                            minecraft_uuid: str = None) -> Dict[str, Any]:
        """解除帳號綁定"""
        try:
    ...
    async def _get_account_link(self, discord_id: int = None, 
                              minecraft_uuid: str = None) -> Optional[Dict[str, Any]]:
        """獲取帳號綁定信息"""
        try:
            if not discord_id and not minecraft_uuid:
                return None
    ...
    async def sync_user_economy(self, discord_id: int, guild_id: int, 
                              direction: str = "to_minecraft") -> Dict[str, Any]:
        """同步用戶經濟數據"""
        try:
            # 獲取帳號綁定
    ...
    async def _sync_user_data(self, discord_id: int, minecraft_uuid: str, guild_id: int):
        ...
        try:
            # 獲取Discord數據
            discord_economy = await self.economy_manager.get_user_economy(discord_id, guild_id)
            discord_achievements = await self.achievement_manager.get_user_achievements(discord_id, guild_id)
    ...
    async def _convert_to_minecraft_format(self, discord_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            minecraft_data = {
                ...
                "level": await self.economy_manager.calculate_level(discord_data.get("experience", 0)),
                "stats": {
    ...
    async def _convert_from_minecraft_format(self, minecraft_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            experience = int(items.get(self.minecraft_items["experience"], 0) / self.exchange_rates["experience"])
    ...
    async def create_transaction(self, from_platform: PlatformType, to_platform: PlatformType,
                               user_id: str, transaction_type: TransactionType, 
                               currency_type: str, amount: int, reason: str,
