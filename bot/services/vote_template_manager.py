...
class VoteTemplateManager:
    ...
    async def apply_template(self, template_id: int, custom_values: Optional[Dict[str, str]] = None) -> Optional[Dict[str, Any]]:
        """應用模板，生成投票配置"""
        try:
    ...
    async def get_templates_by_category(self, category: str, guild_id: Optional[int] = None, 
    ...
    async def search_templates(self, query: str, guild_id: Optional[int] = None, 
                              user_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """搜尋模板"""
        try:
            all_templates = []
