...
class StatisticsManager:
    ...
    async def get_comprehensive_statistics(
        self, guild_id: Optional[int] = None, days: int = 30
    ) -> Dict[str, Any]:
        """獲取綜合統計資料（簡化版）"""
        try:
            # 基本票券統計
    ...
    async def generate_statistical_report(
        ...
        try:
            ...
            if "error" not in stats:
                ...
                report_summary = {
                    "period_summary": {
                        "total_tickets": stats.get("ticket_statistics", {})
                        .get("summary", {})
                        .get("total_tickets", 0),
                        "resolution_rate": stats.get("ticket_statistics", {})
                        .get("summary", {})
                        .get("resolution_rate", 0),
                        "total_votes": stats.get("vote_statistics", {})
                        .get("summary", {})
                        .get("total_votes", 0),
                        "system_status": stats.get("system_statistics", {})
                        .get("summary", {})
                        .get("status", "unknown"),
                    }
                }
    ...
    async def generate_comprehensive_report(
        ...
        try:
            ...
            # 生成關鍵指標
            ticket_stats = comprehensive_stats.get("ticket_statistics", {}).get(
                "summary", {}
            )
            vote_stats = comprehensive_stats.get("vote_statistics", {}).get(
                "summary", {}
            )
            system_stats = comprehensive_stats.get("system_statistics", {}).get(
                "summary", {}
            )
            ...
            async with self.db.connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    return {
                        ...
                        "open_tickets": (
                            tickets_result["open_tickets"] if tickets_result else 0
                        ),
                        "pending_tickets": (
