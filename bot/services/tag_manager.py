...
class TagManager:
    ...
    def __init__(self, tag_dao: TagDAO = None):
        ...
        # 預設系統標籤
        self.system_tags = [
            {'name': 'urgent', 'display_name': '緊急', 'color': '#e74c3c', 'emoji': '🚨', 'category': 'priority'},
            {'name': 'bug', 'display_name': '錯誤回報', 'color': '#e67e22', 'emoji': '🐛', 'category': 'system'},
            {'name': 'feature', 'display_name': '功能請求', 'color': '#3498db', 'emoji': '✨', 'category': 'system'},
            {'name': 'question', 'display_name': '一般問題', 'color': '#f39c12', 'emoji': '❓', 'category': 'system'},
            {'name': 'resolved', 'display_name': '已解決', 'color': '#27ae60', 'emoji': '✅', 'category': 'status'},
            {'name': 'pending', 'display_name': '等待回覆', 'color': '#f1c40f', 'emoji': '⏳', 'category': 'status'},
        ]

    # ========== 標籤創建與管理 ==========
    
    async def create_tag(self, guild_id: int, name: str, display_name: str,
                        color: str = None, emoji: str = None, description: str = None,
                        category: str = "custom", created_by: int = None) -> Tuple[bool, str, Optional[int]]:
        """創建新標籤"""
        try:
            # 驗證標籤名稱
    ...
    async def update_tag(self, tag_id: int, **kwargs) -> Tuple[bool, str]:
        ...
        try:
            # 驗證更新欄位
            valid_fields = ['display_name', 'color', 'emoji', 'description', 'category', 'is_active']
    ...
    async def add_tag_to_ticket(self, ticket_id: int, tag_id: int, added_by: int) -> Tuple[bool, str]:
        """為票券添加標籤"""
        try:
            # 檢查標籤是否存在
            tag = await self.tag_dao.get_tag_by_id(tag_id)
    ...
    async def apply_auto_tags(self, guild_id: int, ticket_id: int, ticket_type: str,
                             content: str, user: discord.Member = None) -> List[Dict[str, Any]]:
        """應用自動標籤規則"""
        try:
    ...
    async def create_auto_rule(self, guild_id: int, rule_name: str, tag_id: int,
                              ...
                              priority: int = 1) -> Tuple[bool, str, Optional[int]]:
        """創建自動標籤規則"""
        try:
            # 驗證觸發類型
    ...
    async def get_tag_statistics(self, guild_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # 熱門標籤
            popular_tags = sorted(usage_stats, key=lambda x: (x['usage_count'] or 0), reverse=True)[:10]
    ...
    async def initialize_default_tags(self, guild_id: int, created_by: int) -> Tuple[bool, str, int]:
        """初始化預設標籤"""
        try:
            created_count = 0
    ...
    def get_category_info(self, category: str) -> Dict[str, str]:
        """取得分類資訊"""
        return self.default_categories.get(category, {
            'name': category,
            'emoji': '🏷️',
            'color': '#808080'
        })
