...
class DataManagementService:
    ...
    def __init__(self):
        ...
        self.table_policies = {
            ...
            # 業務數據 - 中期保留
            "business_data": {
                "tables": ["tickets", "votes", "vote_responses", "workflows", "workflow_executions"],
            ...
            # 系統日誌 - 安全需要
            "system_logs": {
                "tables": ["ticket_logs", "security_events", "data_access_logs", "guild_event_logs"],
                "retention": DataRetentionPolicy.LONG_TERM,
                "requires_consent": False,
                "auto_anonymize": True
            },
            ...
            # 設定數據 - 中期保留
            "configuration_data": {
                "tables": ["ticket_settings", "vote_settings", "welcome_settings", "guild_settings"],
                "retention": DataRetentionPolicy.LONG_TERM,
                "requires_consent": False,
                "auto_anonymize": False
            }
    ...
    async def _export_table_data(self, table: str, guild_id: int, 
                                export_request: DataExportRequest) -> List[Dict]:
        """導出特定表格數據"""
        try:
            # 建構查詢條件
            additional_where = {}
    ...
    async def delete_guild_data(self, guild_id: int, user_id: int,
                        ...
                        try:
                            if hard_delete or data_type == "personal_data":
                                ...
                                if policy["auto_anonymize"]:
                                    anonymized_count = await self._anonymize_table_data(table, guild_id)
                                    deletion_summary["retained_records"][table] = f"{anonymized_count} 筆已匿名化"
                                else:
                                    archived_count = await self._archive_table_data(table, guild_id)
                                    deletion_summary["retained_records"][table] = f"{archived_count} 筆已歸檔"
    ...
    async def _anonymize_if_needed(self, row: Dict, table: str) -> Dict:
        ...
        if "discord_id" in row:
            row["discord_id"] = f"anon_{hashlib.md5(str(row['discord_id']).encode()).hexdigest()[:8]}"
        if "username" in row:
    ...
    async def _log_export_event(self, export_request: DataExportRequest, 
                               exported_categories: int, error: str = None):
        """記錄導出事件"""
        try:
            event_data = {
                "guild_id": export_request.guild_id,
