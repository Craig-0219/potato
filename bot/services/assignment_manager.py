...
class AssignmentManager:
    ...
    async def assign_ticket(self, ticket_id: int, assigned_to: int, assigned_by: int,
                          method: str = "manual", reason: str = "手動指派") -> Tuple[bool, str]:
        """指派票券給客服人員"""
        try:
            # 取得票券資訊
            ...
            if success:
                ...
                await self.assignment_dao.update_staff_workload(
                    ...
                    increment_assigned=True
                )
                
                # 如果原本有指派其他人，減少原客服的工作量
                if original_assigned and original_assigned != assigned_to:
                    original_workload = await self.assignment_dao.get_staff_workload(guild_id, original_assigned)
                ...
                logger.info(f"票券 #{ticket_id} 指派給客服 {assigned_to} by {assigned_by} ({method})")
    ...
    async def auto_assign_ticket(self, ticket_id: int, assigned_by: int) -> Tuple[bool, str, Optional[int]]:
        """自動指派票券"""
        try:
            # 取得票券資訊
            ticket = await self.ticket_dao.get_ticket_by_id(ticket_id)
    ...
    async def _assign_by_specialty(self, guild_id: int, ticket_type: str, max_concurrent: int) -> Optional[int]:
        """根據專精指派"""
        # 先嘗試找專精匹配的客服
    ...
    async def get_staff_workload_summary(self, guild_id: int) -> List[Dict[str, Any]]:
        """取得所有客服工作量摘要"""
        try:
            available_staff = await self.assignment_dao.get_available_staff(guild_id, 999)  # 取得所有客服
            ...
            for staff in available_staff:
                ...
                if workload:
                    ...
                    if workload['total_assigned'] > 0:
                        completion_rate = (workload['total_completed'] / workload['total_assigned']) * 100
    ...
    async def add_staff_specialty(self, guild_id: int, staff_id: int, specialty_type: str, 
                                skill_level: str = "intermediate") -> Tuple[bool, str]:
        """添加客服專精"""
        try:
            # 驗證技能等級
    ...
    async def get_staff_profile(self, guild_id: int, staff_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if workload['avg_completion_time'] > 0:
                ...
                profile['performance_metrics']['efficiency_score'] = (time_score + completion_score) / 2
    ...
    async def get_assignment_analytics(self, guild_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_completion_rate = sum(staff['completion_rate'] for staff in workload_summary) / len(workload_summary) if workload_summary else 0
