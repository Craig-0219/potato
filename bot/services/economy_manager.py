...
class EconomyManager:
    ...
    async def enable_cross_platform_sync(self, guild_id: int, minecraft_api_endpoint: str, 
                                       minecraft_server_key: str) -> bool:
        """啟用跨平台同步"""
        try:
            settings = await self.get_economy_settings(guild_id)
    ...
    async def trigger_cross_platform_sync(self, user_id: int, guild_id: int):
        ...
        try:
            ...
            if last_sync and (datetime.now(timezone.utc) - last_sync).total_seconds() < settings.sync_interval:
                return
    ...
    async def _perform_cross_platform_sync(self, user_id: int, guild_id: int, settings: EconomySettings):
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if result.get("status") == "success":
                            ...
                            # 處理 Minecraft 獎勵加成
                            if result.get("adjustments") and result["adjustments"].get("bonus_coins", 0) > 0:
                                bonus_amount = int(result["adjustments"]["bonus_coins"])
                                await self.add_currency(
                                    user_id, guild_id, CurrencyType.COINS, bonus_amount,
                                    TransactionType.BONUS, "Minecraft 服務器獎勵加成"
                                )
                                logger.info(f"🎁 Minecraft 獎勵加成：用戶 {user_id} 獲得 {bonus_amount} 金幣")
                            ...
                            logger.error(f"❌ Zientis 同步失敗：{result.get('message', '未知錯誤')}")
    ...
    async def _apply_minecraft_adjustments(self, user_id: int, guild_id: int, 
                                         adjusted_balances: Dict[str, int]):
        ...
        try:
            ...
            for currency, amount in adjusted_balances.items():
                ...
                if current_amount != amount:
                    ...
                    # 記錄調整交易
                    await self._record_sync_transaction(
                        user_id, guild_id, "minecraft_adjustment", 
                        {currency: difference}, f"Minecraft端調整：{currency}"
                    )
            
        except Exception as e:
            logger.error(f"❌ 應用Minecraft調整失敗: {e}")

    async def _record_sync_transaction(self, user_id: int, guild_id: int, 
                                     sync_type: str, balances: Dict[str, int],
    ...
    async def handle_minecraft_webhook(self, webhook_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if event_type == "player_activity":
                ...
                # 記錄 Minecraft 活動交易
                await self._record_sync_transaction(
                    user_id, guild_id, "minecraft", 
                    {activity_type: reward_amount}, 
                    f"Minecraft活動：{activity_type}"
                )
                
                logger.info(f"✅ 處理Minecraft活動：用戶 {user_id} {activity_type} +{reward_amount}")
    ...
    async def get_cross_platform_statistics(self, guild_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 統計跨平台交易
            guild_transactions = [
                t for t in self._cross_platform_transactions 
            ...
            stats = {
                ...
                "platform_distribution": {
                    "discord": len([t for t in guild_transactions if "discord" in t.source_platform]),
                    "minecraft": len([t for t in guild_transactions if "minecraft" in t.source_platform])
                }
            }
    ...
    async def perform_anti_inflation_adjustment(self, guild_id: int):
        ...
        try:
            ...
            if inflation_rate > settings.inflation_threshold:
                ...
                settings.voice_coins_per_minute = max(1, int(settings.voice_coins_per_minute * adjustment_factor))
                ...
                settings.voice_coins_per_minute = min(20, int(settings.voice_coins_per_minute * adjustment_factor))
    ...
    async def _update_balances_from_minecraft(self, user_id: int, guild_id: int, balances: Dict[str, float]):
        """從 Minecraft 更新餘額"""
        try:
            for currency, amount in balances.items():
                if currency == "coins":
                    ...
                    async with db_pool.acquire() as conn:
                        ...
                        """, int(amount), datetime.now(timezone.utc), user_id, guild_id)
    ...
    async def setup_zientis_integration(self, guild_id: int, api_endpoint: str, server_key: str) -> bool:
        """設置 Zientis 整合 (修復版)"""
        try:
            # 測試連接 (修復 SSL 問題)
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.get(
                    ...
                    logger.info(f"🔗 Zientis API 健康檢查通過: {health_data.get('status', 'unknown')}")
