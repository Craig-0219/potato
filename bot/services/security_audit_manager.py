...
class SecurityAuditManager:
    ...
    async def log_data_access(
        ...
        await self.log_security_event(
            ...
            guild_id=guild_id,
            action=(
                AuditAction(operation.lower())
                if operation.lower() in [a.value for a in AuditAction]
                else AuditAction.ACCESS
            ),
    ...
    async def _detect_suspicious_activity(self, event: SecurityEvent):
        ...
        self.suspicious_activity_cache[user_key] = [
            t
            for t in self.suspicious_activity_cache[user_key]
            if t > cutoff_time
        ]
    ...
    async def _evaluate_anomaly_rule(
        ...
        """評估異常規則"""
        if (
            conditions.get("check_elevation")
            and event.event_type == SecurityEventType.ROLE_ASSIGNMENT
        ):
            # 檢查是否為權限提升
    ...
    async def _send_security_alert(
        self, rule: SecurityRule, event: SecurityEvent, action: Dict[str, Any]
    ):
        """發送安全警報"""
    ...
    async def _apply_temporary_restriction(
        self, user_id: int, action: Dict[str, Any]
    ):
        """應用臨時限制"""
        duration = action.get("duration", 1800)  # 預設30分鐘
        restriction_end = datetime.now(timezone.utc) + timedelta(
            seconds=duration
        )
            ...
            if isinstance(value, str):
                ...
                for pattern in sensitive_patterns:
                    sanitized_value = re.sub(
                        pattern, "[REDACTED]", sanitized_value
                    )
    ...
    async def generate_compliance_report(
        ...
        try:
            ...
            # 生成摘要
            summary = await self._generate_compliance_summary(
                relevant_events, standard
            )
            ...
            logger.info(
                f"✅ 合規報告已生成: {standard.value} for guild {guild_id}"
            )
            return report
    ...
    async def _analyze_compliance_violations(
        self, events: List[SecurityEvent], standard: ComplianceStandard
    ) -> List[Dict[str, Any]]:
        """分析合規違規"""
        violations = []
        ...
        if standard == ComplianceStandard.GDPR:
            ...
            for event in data_access_events:
                if (
                    ...
                    and "personal_data" in str(event.details)
                ):
                    violations.append(
                        {
        ...
        return {
            ...
            "user_management_events": len(
                [
                    e
                    for e in events
                    if e.event_type == SecurityEventType.USER_MANAGEMENT
                ]
            ),
    ...
    async def _generate_compliance_recommendations(
        self, violations: List[Dict[str, Any]], standard: ComplianceStandard
    ) -> List[str]:
        """生成合規建議"""
        recommendations = []
    ...
    async def get_security_statistics(
        ...
        return {
            ...
            "events_by_risk_level": self._count_events_by_risk_level(
                relevant_events
            ),
    ...
    def _get_top_users_by_activity(
        ...
        return [
            {"user_id": user_id, **stats} for user_id, stats in sorted_users
        ]
