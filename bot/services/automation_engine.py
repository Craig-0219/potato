...
class AutomationEngine:
    ...
    async def get_rules(
        self, guild_id: int = None, status: RuleStatus = None
    ) -> List[AutomationRule]:
        """獲取自動化規則列表"""
    ...
    async def process_event(
        self, event_type: TriggerType, event_data: Dict[str, Any]
    ...
    async def _find_matching_rules(
        ...
        for rule in self.rules.values():
            ...
            if (
                ...
                or rule.trigger.type != event_type
                or rule.guild_id != guild_id
            ):
                continue
