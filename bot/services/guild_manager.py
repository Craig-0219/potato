...
from bot.services.guild_permission_manager import (
    guild_permission_manager, GuildRole, GuildPermission
)
import aiomysql
...
class GuildManager:
    ...
    async def on_guild_join(self, guild: discord.Guild):
        """處理 Bot 加入新伺服器"""
        try:
            logger.info(f"🆕 Bot 加入新伺服器: {guild.name} (ID: {guild.id}, 成員: {guild.member_count})")
            ...
            async with self._initialization_lock:
                ...
                if success:
                    ...
                    await self._log_guild_event(
                        guild.id, None, "guild_join", "guild_management",
                        "Bot joined guild", {"member_count": guild.member_count}
                    )
                else:
    ...
    async def on_guild_remove(self, guild: discord.Guild):
        ...
        try:
            ...
            # 記錄離開事件
            await self._log_guild_event(
                guild.id, None, "guild_leave", "guild_management",
                "Bot left guild", {}
            )
    ...
    async def on_guild_update(self, before: discord.Guild, after: discord.Guild):
        ...
        try:
            ...
            if changes:
                ...
                # 記錄更新事件
                await self._log_guild_event(
                    after.id, None, "guild_update", "guild_management",
                    f"Guild updated: {', '.join(changes)}", 
                    {"before": self._serialize_guild(before), "after": self._serialize_guild(after)}
                )
    ...
    async def _create_guild_info(self, guild: discord.Guild):
        ...
        try:
            guild_data = {
                ...
                "preferred_locale": str(guild.preferred_locale) if guild.preferred_locale else "zh-TW",
    ...
    async def _log_guild_event(self, guild_id: int, user_id: Optional[int], 
                              event_type: str, category: str, 
