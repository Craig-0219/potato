...
class CICDPerformanceAnalyzer:
    ...
    def fetch_workflow_runs(self, days: int = 7, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        params = {"per_page": limit, "branch": "dev", "status": "completed"}
    ...
    def generate_optimization_suggestions(
        self, bottlenecks: List[Dict[str, Any]], success_rates: Dict[str, float]
    ) -> List[str]:
        """生成優化建議"""
        suggestions = []
        ...
        for b in bottlenecks:
            if b["avg_duration"] > 10:
                suggestions.append(
                    f"⚡ {b['workflow']} 平均執行時間過長 ({b['avg_duration']:.1f}分鐘)，建議:"
                )
            ...
            if b["variability"] > 2:
                suggestions.append(
                    f"📊 {b['workflow']} 執行時間變異大 (±{b['variability']:.1f}分鐘)，需要穩定性優化"
                )
...
def main():
    ...
    parser.add_argument(
        "--repo", default="Craig-0219/potato", help="GitHub 倉庫 (默認: Craig-0219/potato)"
    )
