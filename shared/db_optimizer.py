...
class DatabaseOptimizer:
    ...
    async def analyze_query(self, query: str, params: tuple = None) -> QueryAnalysis:
        ...
        try:
            ...
            # 執行查詢並測量性能
            execution_time, rows_examined, rows_sent = await self._execute_and_measure(query, params)
    ...
    async def _execute_and_measure(self, query: str, params: tuple = None) -> Tuple[float, int, int]:
        """執行查詢並測量性能"""
        start_time = time.time()
        rows_examined = 0
        rows_sent = 0
    ...
    async def _generate_optimization_suggestions(
        self, explain_result: Dict, execution_time: float, 
        query_type: QueryType, query: str
                    ...
                    for metric_name, query in metrics_queries.items():
                        try:
                            ...
                            if metric_name == 'connections':
                                ...
                                metrics_data[metric_name] = float(result[0]) if result and result[0] else 0.0
                ...
                async with conn.cursor() as cursor:
                    ...
                    return {
                        ...
                        'slow_queries': {
                            ...
                            'avg_time': float(slow_query_stats[1]) if slow_query_stats and slow_query_stats[1] else 0,
                            'max_time': float(slow_query_stats[2]) if slow_query_stats and slow_query_stats[2] else 0,
