...
class WorkflowCore(commands.Cog):
        ...
        try:
            ...
            if not interaction.user.guild_permissions.manage_guild:
                await interaction.response.send_message(
                    "❌ 需要管理伺服器權限才能創建工作流程", ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                f"❌ 創建工作流程失敗: {str(e)}", ephemeral=True
            )
    ...
    async def execute_workflow(self, interaction: discord.Interaction, workflow_name: str):
        ...
        try:
            ...
            if not target_workflow:
                await interaction.response.send_message(
                    f"❌ 找不到工作流程: {workflow_name}", ephemeral=True
                )
                return
    ...
    async def workflow_status(self, interaction: discord.Interaction, execution_id: str):
        ...
        try:
            ...
            if not status:
                await interaction.response.send_message(
                    f"❌ 找不到執行記錄: {execution_id}", ephemeral=True
                )
                return
    ...
    async def toggle_workflow(self, interaction: discord.Interaction, workflow_name: str):
        ...
        try:
            ...
            success = await workflow_engine.update_workflow(
                target_workflow["id"], {"status": new_status}
            )
    ...
    async def create_workflow_template(
        self, interaction: discord.Interaction, template_type: str, name: str
    ):
        """創建工作流程模板"""
        try:
            # 檢查權限
            ...
            if not template:
                await interaction.response.send_message(
                    f"❌ 未知的模板類型: {template_type}", ephemeral=True
                )
                return
