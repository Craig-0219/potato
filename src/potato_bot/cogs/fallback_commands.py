...
class FallbackCommands(commands.Cog):
    ...
    async def menu_fallback(self, ctx):
        """開啟主選單 (備用命令)"""
        try:
            # menu_core 已移除，直接給出可用指令提示
            embed = discord.Embed(
                title="🏠 Potato Bot 主選單 (簡易版)",
                description="目前 GUI 選單未啟用，請使用以下指令：",
                color=0x00FF88,
            )
    ...
    async def ticket_fallback(self, ctx):
        """票券系統 (備用命令)"""
        try:
            ticket_cog = self.bot.get_cog("CachedTicketCore")
            if ticket_cog:
                embed = discord.Embed(
                    title="🎫 票券系統",
                    description="票券系統功能",
    ...
    async def status_fallback(self, ctx):
        ...
        try:
            ...
            latency = (
                f"{round(self.bot.latency * 1000)}ms" if self.bot.latency is not None else "N/A"
            )
